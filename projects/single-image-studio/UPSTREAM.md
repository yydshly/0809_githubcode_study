...
Slice 11 当前仅新增本地原创 scope 合同与空 workspace README，计划版本化 expected projection 和 worker lifecycle；没有新增 dependency、upstream package、第三方代码 / 图片 / 模型 / 权重、machine definition、result 或产品接线，也没有运行 Sharp。它只能引用既有项目原创 public-synthetic material 与已锁定 runtime lineage，后续任何真实 calibration 都必须等待新的 results-zero definition 提交推送。
