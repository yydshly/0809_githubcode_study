...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 06 唯一注册诊断已闭合；Slice 07 results-zero definition 已冻结 | `Slice 07: definition-frozen / results-zero / Gate-B-smoke-not-run / calibration-forbidden / non-C1 / non-product` | 16 schemas / 23 non-index records / 12 source wrappers / 36 planned attempts；中央校验和再生成通过，真实 Slice 07 Sharp 图片路径未运行 |
...
| Slice 07 新候选定义 | results-zero definition 已冻结；真实 smoke 未运行 | freeze `2026-08-15T12:15:57.873Z`；full tree `bc608ed9…0481`；16 schemas / 24 records / 12 wrappers | encoder 经 independent oracle 重开；worker 只返回 RGBA8，driver 固定 12 sources / 36 attempts。results=0、calibration 禁止、全轴 0。详见 [Slice 07 evidence](research/SLICE_07_EVIDENCE.md) |
...
9. **Slice 07 results-zero definition 已冻结，不得重跑 Slice 06**：新版本选择 Sharp 解码 / 像素处理 + candidate-owned canonical PNG encoder，且生产 encoder 与 oracle 实现隔离。16 schemas、23 non-index records、12 source wrappers 与 36-attempt denominator 已封存；只有该定义基线提交并推送后才允许一次 registered Gate-B smoke。当前 Gate B、calibration、C1、产品支持继续为 0 / false。
