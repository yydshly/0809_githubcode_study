...
results-zero definition 已以真实 UTC `2026-08-15T08:17:06.288Z` 冻结：26 schemas、23 个非 index records、49 descendants、50 machine files，连同 README 为 51 files；index content / file SHA-256 为 `d537199c8bc6147761da297daeddb03e1ff837a83c8d2c57af29c9e5b9b67e08` / `1cb934a1d870a62e9ccb706e3c21dcdbb54de55f027a325e31230ac4bf3cb20c`，full-tree SHA-256 为 `19a42a2e63b73fb0971e7038e4470c02551dce9b0ddba2dfc87dd3e5780d47b3`。central `31 / 31`、Slice 06 六套 `95 / 95`、整仓 verify `323 / 323`；冻结时 results 与 copied image bytes 均为 0，未运行真实 Sharp。

定义所在提交必须先提交、推送，并在 clean Git 状态下通过 driver admission，之后一次 driver invocation 才可按顺序注册 normalize / export 各一个 operation run。首项若 protocol-failed / inconclusive 或任何 drift / reconciliation unknown 会全局停止，actual 计数不得冒充 planned 8 / 24，也不得 retry。所有 output 只可成为排除于 Gate B 的 diagnostic specimen / quarantine，并必须绑定完整 worker 与 independent-oracle 子记录。无论结果如何，Slice 06 都保持 `Gate-B-not-entered-diagnostic-only`、`calibrationAuthorized=false`；下一切片才可根据诊断选择 / 修正 candidate 并重新冻结 Gate-B smoke。Slice 06 不创建 calibration、正式 holdout、defect-holdout、escape、产品 UI / server 接线、真实图片或模型权重。
