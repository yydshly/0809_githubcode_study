...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 10 已 fail closed；Slice 11 Phase B protocol + durable foundation 已实现 | `Slice 11: scope-frozen / projection-case-lifecycle-protocol-and-durable-foundation / definition-not-created / calibration-not-run / non-C1 / non-product` | projection、case、lifecycle、request / terminal / ledger / summary、durable claim / runtime observation、applicable / rejection / failure closure 与 48×3 durable attempt bridge 通过 33 / 33 fake-only tests；真实 calibration 仍须等待 durable event ledger / operation close、central validator 与新 results-zero definition 提交推送 |
...
| Slice 11 foundation | `@0.11.0`；projection / adapter / case / lifecycle / protocol runner / durable attempt bridge 已实现，definition / results 未创建 | 新身份的 96 sources / 288 attempts；零 retry / replacement | 11 strict schemas；request 先持久化再幂等 claim；applicable 六文件、rejection 三文件、pre-worker failure 两文件 closure；runner 必须等发布完成才进入下一 slot；33 / 33 fake-only。详见 [Slice 11 合同](research/SLICE_11_CONTRACT.md) |
...
13. **Slice 11 protocol + durable foundation 已实现，definition 尚未创建**：`@0.11.0` 的 projector、adapter 与 case executor 保留完整 gold identity并只向底层下发 exact 12 fields；strict lifecycle record 独立记录 preflight、spawn、IPC 与 exit。runner 固定每 operation 48×3、零 replacement。durable attempt bridge 先以 `wx` 持久化 request 并 claim，再执行一次 worker；applicable 发布六文件 closure，rejection 发布 lifecycle / terminal / publication，pre-worker failure 发布 terminal / publication。每个 closure 完成前 runner 不得进入下一 slot；post-rename 不确定性保留唯一 closure 并全局停止，重放不获得执行权。33 / 33 fake-only tests已覆盖完整 48×3 临时目录闭包。下一步仍需 durable publication-event ledger、runtime / summary operation close、central validator 与 machine definition；冻结并推送 results-zero baseline 后才可能运行。它仍不是 C1 或产品支持。
