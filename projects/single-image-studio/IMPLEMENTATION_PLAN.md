...
“基础功能冻结”不等于产品功能完成。当前新增可执行的场景技能层：商品白底图、报名照 / 底色头像、社交头像与封面只编排已经存在的 renderer、Provider、蒙版与下载合同，不引入新模型。商品场景默认白底并强制不透明 JPEG；报名照场景明确不是证件合规工具；社交场景完全本地。老照片修复、主体感知布局和正式证件规格必须另立能力，不得用当前全局调色或模板冒充。
...
8. 继续界面收敛：补三场景的真实浏览器视觉排序、键盘 / 指针与系统下载复核；随后用场景任务而非孤立控件组织早期内部走查；
9. 老照片修复保持未实现。只有另行选择恢复模型 / 服务、许可与质量分母后，才建立独立场景，不复用“自然增强”名称偷升能力。
