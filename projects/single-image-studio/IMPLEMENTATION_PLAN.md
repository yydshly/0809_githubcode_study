...
截至 2026-08-15，Slice 02 已完成四份研究 `CapabilityContract@0.2.0`、fixture reference adapter 与两套 suite 的五 partition 结构隔离骨架；Slice 03 完成 15 行 `NORMALIZE-DELIVER` 格式政策 / profile、25 个开放项目原创夹具、独立 byte-backed technical observer，以及只用临时 mock metadata 的密封仪式 rehearsal。Slice 04 又把 Sharp `v0.35.3`、Windows x64 bundle、`sharp-libvips@1.3.2` 与 bundled libvips 上游边界锁成一个 source-resolved 复合候选，并冻结两份 metadata-only 合同、QA、normalize / export 两份 operation-specific 五 partition plan、两份预注册与 seal intent。Slice 05 已按 [范围合同](research/SLICE_05_CONTRACT.md) 于 `2026-08-15T04:23:38.389Z` 完成 machine definition freeze：installed runtime closure、两份 `CapabilityContract@0.5.0`、artifact / result schemas、independent oracle / gold、adapter / isolated worker、named hardware、local runner / fault semantics、Gate B plan、6 份 manifest 与两份开放 calibration 预注册均已封口；canonical results 为 0，真实 Sharp smoke、Gate B 决定、开放 calibration、formal runner / trust / roles 与正式像素仍未发生，因此 WP3 与任何 C1 判定仍未开始。

立即后续只能先提交并推送 Slice 05 exact definition baseline，再从该不可变 baseline 对 normalize / export 分别运行真实 Sharp smoke。只有某项 operation 的 Gate B smoke 全项通过后，才可运行该项项目原创开放 `dev/calibration=30` / `defect/calibration=18`；不得由另一项继承，也不得创建正式 holdout。calibration 若改变 candidate、QA、阈值或停止规则，必须发布新版本和新预注册。进入 Matting 候选、`MATTE-REAL`、真实照片、模型权重或产品 UI 仍需另行明确授权。Slice 04 的完成边界与验证结果见 [范围合同](research/SLICE_04_CONTRACT.md) 和 [证据记录](research/SLICE_04_EVIDENCE.md)；Slice 05 的历史硬边界与当前定义事实分别见 [范围合同](research/SLICE_05_CONTRACT.md) 和 [定义冻结证据](research/SLICE_05_EVIDENCE.md)。
...
这在 Slice 04 关闭时不是 Gate B：当时 operation artifact schema / independent oracle / gold、adapter、执行位置、named hardware、资源 guardrail、幂等 / timeout / retry / reconciliation、实际输入输出 smoke 与开放 calibration 都未建立。Slice 05 现已把除实际 smoke / decision / calibration 外的开放定义条件冻结为新 `@0.5.0` 版本；这不倒写 Slice 04，也不授予 Gate B。正式 bundle 和一次性正式运行还要等待 actual smoke、通过后的开放 calibration，以及 calibration 后新最终版本、阈值与预注册的独立审计。

### 2.2 Slice 05 定义已冻结，执行未开始
...
冻结树含 25 份 strict schema、6 份 operation-specific manifest、108 份 source provenance / raw asset、54 份 independent export input artifact 与 54 份 independent gold，且 results 为 0。normalize 与 export 的 artifact / oracle / gold、adapter binding、hardware / runtime、运行语义、开放 fixture manifest 与 machine preregistration 已分别固定；实际 input / output / failure smoke 没有运行。某项 smoke 任一硬门 non-pass 时，该项保持非 Gate B 且不得运行 calibration；只有全项通过时，才能运行该项 `dev/calibration=30` 与 `defect/calibration=18`，每来源 3 次 planned repetitions。完整开放 calibration 计划为两项 operation 共 96 个独立来源、288 个 planned repetitions；另有 12 个 smoke 来源，永不进入 calibration 分母。

当前先把定义基线提交 / 推送，再运行 operation-specific smoke；不得先运行后补提交。所有格式继续 `productSupport=false`，smoke 与开放 calibration 都不进入未来 C1 分母，也不建立 EvidenceManifest。正式 holdout、defect-holdout、escape、bundle、request、receipt、formal runner / trusted authority 继续为 `not-created`；C1、U1、E1、R1-pipeline、R1-product-validation、R1-product-release、O1、G1、V1 与 Release Gate 全部维持 0。完整验收与非能力边界见 [Slice 05 定义冻结证据](research/SLICE_05_EVIDENCE.md)。
