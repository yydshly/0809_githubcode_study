...
| `REG-NORM-SHARP-CANONICAL-PNG@0.11.0` | Slice 10 immutable failure lineage + versioned expected projection / truthful lifecycle | `CAP-02` | canonical PNG normalize / export | `scope-frozen/fake-only-protocol-runner-foundation` | projection / case / lifecycle 与纯内存 request-terminal-ledger-summary runner 通过 20 / 20 fake-only tests；durable publication、96/288 definition 与 canonical results 尚未创建；零 retry/replacement，非 C1、非产品支持 |
...
8. Slice 11 / `@0.11.0` scope 已冻结；projection、事实派生 lifecycle、frozen-material case executor，以及 versioned request / terminal / ledger / summary 的纯内存 runner foundation 已通过 20 / 20 fake-only tests。它不调用 Sharp；terminal 只能通过 lifecycle / projection 交叉绑定证明 worker 事实，ledger 与 summary 不能靠自重哈希洗白，rejection 保持 worker-free。durable claim / publication、machine definition 与 canonical results 仍未创建；完整实现和 results-zero 基线分别提交推送前不得运行。
