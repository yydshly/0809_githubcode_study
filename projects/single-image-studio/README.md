...
用户也可以在结果页选择“换个处理方向”：当前图片保留，旧结果与下载资格退出，任务配置和旧 run 被状态机清理，然后返回分组后的任务列表。设置页和错误页使用同一返回规则，避免界面已经回到任务列表、后台却仍保留旧任务的状态分叉；返回本身不会上传或执行图片。
...
基础链路可用不等于所有模块完成。页面中的创意生成已有真实 API 路径，但当前服务未配置；此前只有占位卡的“标准底色头像”现已收窄为可运行的**通用底色头像**：先在本机以完整预览确定 1:1 / 4:5 构图和整体光色，再逐次确认远程抠图，最后在本机修边、选择底色并导出不透明 JPEG。它复用现有 Provider，不新增模型，也不承诺证件或机构受理。**自然增强**也已成为独立本地流程：五个固定预设只改变页面上可见的亮度、对比度与饱和度，仍可手调、比较和可靠下载；它不分析图片内容，也不冒充自动质量优化。场景尺寸模板已提供透明的构图起点；批量、主体感知裁切、扩图和正式证件规格仍未实现。
...
内部浏览器诊断页位于 `http://127.0.0.1:4177/browser-diagnostics.html`。它不会读取用户图片或访问网络，而是在当前浏览器内用内存像素核对 PNG / JPEG 编码、metadata、独立重开、两笔蒙版修正后的透明 PNG 与纯色 JPEG，以及 16:9 / 9:16 场景模板的真实 Canvas 输出和小图不放大规则；同时用真实 DOM 焦点检查远程失败、未知状态和本地失败三种恢复操作。通过只代表当前浏览器会话，不代替主产品页的真实画笔、模板点击、拖动、布局和下载交互验收。

主产品本地流程的自驱验收页位于 `http://127.0.0.1:4177/product-acceptance.html`。它在同源 iframe 中以项目合成图实际完成 1280×720 的“保真整理 → 1:1 + 旋转 → PNG”和 1440×900 的“横版封面模板 → PNG”，捕获下载 Blob、核对 PNG 签名并重新解码，同时检查任务分组、关键焦点和流程内 `console.error`。它不会调用远程服务，也不验证操作系统下载目录；在一个浏览器中通过仍不能替代 Chrome / Edge 两个冻结版本的正式矩阵。
