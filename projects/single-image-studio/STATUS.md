...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 08 typed protocol、durable runner 与 results-zero definition stack 已完成 fake-only 验证 | `Slice 08: scope-frozen / implementation-fake-tested / definition-not-created / Gate-B-smoke-not-run / calibration-forbidden / non-C1 / non-product` | 六套定向 28 / 28；Slice 07 保持 closed non-pass；新版本仍须先冻结并推送定义，再完整执行 12 sources / 36 attempts，不拼接或补跑旧结果 |
...
| Slice 08 协议 | typed context、actual-case driver、durable runner、registered driver、definition generator 与 central validator 已实现并 fake-tested | 6 modules / 6 tests；targeted 28 / 28；计划 12 sources / 36 attempts；definition/results=not-created | closed self-hashed context 绑定 disposition / code / source / manifest / gold；rejection worker-free；中央验证重开 lineage、ledger、PNG/oracle 与完整 fake 结果；真实 Sharp 路径禁止到 results-zero definition 提交并推送之后 |
...
10. **Slice 08 implementation stack 已通过 fake-only 28 / 28**：按 [Slice 08 范围合同](research/SLICE_08_CONTRACT.md) 新建 closed self-hashed `caseContext`，实际 callback 只接收 `{caseContext}`；source bytes / hash 与 gold 都需交叉绑定，rejection 不得读取 gold 或启动 worker。durable runner、registered driver、deterministic generator 与 central validator 已闭合，能够重开 16 schemas、Slice 05 / 07 lineage、ledger、candidate output 和 independent oracle；fake 36-attempt post-run 也必须完整通过。不是只补跑 18 个 rejection，而是在新版本、新 ID、新 request 下完整计划 normalize / export 各 18 次。当前 canonical machine definition、result、decision 均未创建，calibration 继续禁止。
11. 更后续才可另行冻结 Matting 范围，扩 `MATTE-GT / MATTE-REAL` 独立来源并锁定许可允许研究、真正输出连续 Alpha 的至少两个候选与可用市场基准。市场基准缺席需事前预注册；simple baseline 只作比较下限，不进入产品 fallback。
12. 跑通“主体区域 → 边缘净化 → 透明主体 → 纯色换底 → 任务 QA”的第一条纵向链。
13. 独立验证自然增强的退化 / no-op 协议，并完成 CR1 / CR2 真实创意对测。
14. 先用真实管线完成形成性研究并冻结验证契约，再由最小冻结研究界面取得 R1-product-validation 和按效果划分的 V1-validation，作为是否进入正式产品设计的决策输入。
15. 只有质量、主体 / 背景、创意三个方向各至少一个效果取得依赖 C1、U1 / E1、R1-pipeline、R1-product-validation、研究目标 O1 / G1、对应验证范围 V1，且研究界面可见资产通过所需 Release Gate，才进入正式桌面浏览器产品界面设计；正式界面仍须另取 R1-product-release，并让发布所用 V1 精确绑定该界面版本，或通过有效 `V1MigrationManifest` 完成等价迁移。
