...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 06 唯一注册诊断已闭合，精确子因已持久化 | `characterization-complete / Gate-B-no-authority / calibration-forbidden / non-C1 / non-product` | normalize / export 各 12 次、零 replacement；18 / 18 applicable output 都是确定性像素正确但 PNG profile non-pass（缺 `sRGB`、含禁止的 `pHYs`），两项 preflight sentinel 均 3 / 3 精确 worker-free 拒绝。`@0.6.0` 禁止重跑；下一切片必须选择新 candidate 并另行冻结 Gate-B smoke |
...
9. **下一步必须新建候选版本，不得重跑 Slice 06**：实际子因已定位为 Sharp PNG 输出共同缺少 `sRGB` chunk 且带有禁止的 `pHYs`；像素 identity 和 filter-0 正确。下一切片可评估新的复合 candidate（Sharp 解码 / 像素处理 + candidate-owned canonical PNG encoder），但必须与 oracle 实现隔离，并重新冻结 candidate、contract、runtime、Gate-B plan、preregistration 与 denominator 后才能运行 Gate-B smoke。当前 Gate B、calibration、C1、产品支持继续为 0 / false。
