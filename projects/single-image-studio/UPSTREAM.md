...
Slice 10 当前新增本地原创 scope 合同、workspace README、只依赖 Node builtins 的 calibration protocol foundation、不提供 materialization CLI 的 definition-preview generator，以及无 CLI / 无 actual adapter 接线的 durable runner foundation。协议 tests 在内存构造严格记录；definition tests 只读取 Sharp 版本 inventory 和仓库内既有项目原创 Slice 05 open-synthetic records，在两个系统临时树构造 176 个 JSON/schema 文件；runner tests 只用短 fake bytes，在系统临时树构造 48×3 terminal / publication 与 432-event ledger，验证逐文件 fsync、原子 rename、零 retry、runtime-end drift hard-stop 后自动丢弃。所有测试均不调用 Sharp 图像管线、不复制或生成 canonical 图片 bytes、不写 canonical definition/results。当前没有新增 dependency、runtime artifact、第三方代码或资产、真实图片、模型、权重、calibration result、formal holdout 或产品接线；任何 Sharp calibration 必须等 registered driver、中央 validator 和 results-zero definition 提交并推送后才可运行。
