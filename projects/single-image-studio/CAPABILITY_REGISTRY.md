...
| `REG-NORM-SHARP-CANONICAL-PNG@0.9.0` | 相同像素 / encoder 架构 + versioned gold identity | `CAP-02` | canonical PNG normalize / export | `scope-frozen/runner-fake-tested` | identity / context / actual driver / durable runner 通过 19 / 19；3+3×3 分母、rejection worker-free、atomic closure、ledger、bytes / ref / manifest / replay 均 fail closed；definition / canonical result 为 0，非 Gate B |
