...
Slice 09 Phase A 只新增本地原创范围文档，冻结 `goldRecordId` 精确绑定与新完整 36-attempt 研究分母。后续本地原创的 gold identity / case context / actual driver 只 import Node builtins 与既有项目内 candidate / oracle validators，以仓库中真实 Slice 05 normalize / export gold JSON 做注入式 fake-only 绑定测试。它们不新增 dependency / upstream package / 第三方代码或资产，不启动 Sharp worker，不创建 machine definition、fixture copy、request、result 或 decision。
