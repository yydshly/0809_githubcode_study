...
基础链路可用不等于所有模块完成。页面中的创意生成已有真实 API 路径，但当前服务未配置；此前只有占位卡的“标准底色头像”现已收窄为可运行的**通用底色头像**：先在本机以完整预览确定 1:1 / 4:5 构图和整体光色，再逐次确认远程抠图，最后在本机修边、选择底色并导出不透明 JPEG。它复用现有 Provider，不新增模型，也不承诺证件或机构受理。自然增强、批量、模板、扩图和正式证件规格仍未实现。
