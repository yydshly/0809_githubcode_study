...
> 状态：**定义已冻结；完成 5 次非计费沙盒调用，正式候选比较尚未开始**。其中 1 次为协议探针，4 次为项目生成挑战图；全部在正式分母之外。核对日期：2026-08-16。本文不授权注册付费套餐、上传真实用户图片或把沙盒结果解释为正式质量结论；密钥只存在于 Git 忽略的本地 `.env`。
...
2026-08-16T16:08:36Z–16:08:44Z，项目随后用内置图像生成工具创建 4 张非用户、非第三方、非真实身份的挑战图，并分别执行一次显式、顺序、零重试的 PhotoRoom sandbox 调用。四次均成功返回 `1254×1254` RGBA PNG，单次耗时 1,614–3,164 ms；完整输入与输出 hash、Alpha 统计和人工复核见 [sandbox-v0 review](provider-evaluation/sandbox-v0/RESEARCH.md)。目视结果均不应直接交付：头发和毛发有明显光晕，透明玻璃有背景色渗漏，藤椅镂空处保留大量原背景。该观察把“协议成功”和“成品质量”明确分开，并直接支持后续保留/擦除 mask 修正能力；它仍不进入冻结 12-source 分母，也不授权普通 Key 或付费调用。
