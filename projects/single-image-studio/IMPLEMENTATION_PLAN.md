...
截至 2026-08-15，Slice 02 已完成四份研究 `CapabilityContract@0.2.0`、fixture reference adapter 与两套 suite 的五 partition 结构隔离骨架；Slice 03 完成 15 行 `NORMALIZE-DELIVER` 格式政策 / profile、25 个开放项目原创夹具、独立 byte-backed technical observer，以及只用临时 mock metadata 的密封仪式 rehearsal。Slice 04 又把 Sharp `v0.35.3`、Windows x64 bundle、`sharp-libvips@1.3.2` 与 bundled libvips 上游边界锁成一个 source-resolved 复合候选，并冻结两份 metadata-only 合同、QA、normalize / export 两份 operation-specific 五 partition plan、两份预注册与 seal intent。Slice 05 已按 [范围合同](research/SLICE_05_CONTRACT.md) 于 `2026-08-15T04:23:38.389Z` 完成 machine definition freeze；其唯一注册真实 smoke 随后关闭为 non-pass：normalize `6/18`、export `9/18` pass，两份 Gate B decision 均为 `denied-not-entered`，artifact 为 0、calibration 禁止。Slice 06 按 [新范围合同](research/SLICE_06_CONTRACT.md) 完成了 24-attempt diagnostic characterization 的 Phase B protocol 实现与 fake-only `42 / 42`，但 machine definition 与真实诊断仍未开始，因此 WP3 与任何 C1 判定仍未开始。

立即后续禁止重跑 `@0.5.0` 或运行其 calibration。先以新的 `@0.6.0` candidate / contracts / diagnostic preregistrations 持久化 output specimen、worker observation、exit state 与精确 oracle 子因，并验证 normalize missing-sRGB 的新错误优先级；这一阶段没有 Gate B decision authority。只有诊断闭合并由下一切片据此选择 / 修正候选后，才可另行冻结真正的 operation-specific Gate-B smoke。进入 Matting 候选、`MATTE-REAL`、真实照片、模型权重或产品 UI 仍需另行明确授权。Slice 04 的完成边界与验证结果见 [范围合同](research/SLICE_04_CONTRACT.md) 和 [证据记录](research/SLICE_04_EVIDENCE.md)；Slice 05 的历史硬边界与当前定义 / smoke 事实分别见 [范围合同](research/SLICE_05_CONTRACT.md) 和 [证据](research/SLICE_05_EVIDENCE.md)。
...
Slice 06 Phase A 冻结 [scope contract](research/SLICE_06_CONTRACT.md)；Phase B 已实现 strict adapter、isolated worker、independent PNG diagnostic oracle、durable runner、3 份落盘 oracle record schema、10 份 runner-exported strict schema document 与 3 组 fake-only tests。oracle / runner / runtime 合计 `42 / 42`，4 个脚本语法检查通过，独立审计 `P1 / P2 / P3 = 0 / 0 / 0`，10-file deterministic set SHA-256 为 `d7e83c8c5a70ce6929ab5d53be473f413f37abe4459a1bd810164e495d2ab76c`。这些测试只在内存构造 synthetic PNG bytes、在系统临时目录构造 fake closure / result tree；没有运行真实 Sharp 图片路径，也没有创建或提交 canonical repository output、fixture wrapper 或 result。

计划身份仍是尚未创建 machine record 的 `REG-NORM-SHARP@0.6.0`、normalize / export 两份 `CapabilityContract@0.6.0` 与两份 operation-specific diagnostic preregistration。计划分母固定为 8 个既有 public-synthetic regression source unit，每项 3 次，共 24 次：normalize / export 各 3 个 opaque / partial-alpha / alpha-holes applicable lineage，另分别增加 missing-sRGB 与 invalid-artifact preflight sentinel。

Phase C 才可新增 generator / central validator，取得 fresh inventory，并以真实 UTC 冻结 results-0 definition；定义必须先通过对抗审计、提交并推送，之后每个 operation 才可运行一次 registered characterization。所有 output 只可成为排除于 Gate B 的 diagnostic specimen / quarantine，并必须绑定完整 worker 与 independent-oracle 子记录。无论结果如何，Slice 06 都保持 `Gate-B-not-entered-diagnostic-only`、`calibrationAuthorized=false`；下一切片才可根据诊断选择 / 修正 candidate 并重新冻结 Gate-B smoke。Slice 06 不创建 calibration、正式 holdout、defect-holdout、escape、产品 UI / server 接线、真实图片或模型权重。
