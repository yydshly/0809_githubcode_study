...
8. Slice 11 / `@0.11.0` scope 已冻结：只允许建立 content-addressed gold-to-adapter exact 12-field expected 投影和事实派生的 worker lifecycle，并以新 identities 重建 96 / 288、零 retry / replacement 的开放分母。implementation、machine definition 与 results 均未创建；scope、实现和 results-zero 基线分别提交推送前不得运行。
