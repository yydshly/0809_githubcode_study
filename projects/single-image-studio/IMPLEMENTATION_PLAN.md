...
> 2026-08-17 短周期补充：基础能力和五条浏览器流程已经形成内部 Alpha 基线。未来 3–5 周的具体顺序、工作包和退出条件以 [PRODUCT_STABILIZATION_ROADMAP.md](PRODUCT_STABILIZATION_ROADMAP.md) 为准；本文件继续保留总体产品路线、架构边界和研究恢复条件。两者冲突时，当前状态先以 `STATUS.md` 判定，近期动作按稳定化路线执行。
...
2026-08-17 增量：验收页已新增 1366×768 的 `UT-OLD-PHOTO` 本地旅程，使用项目原创虚构老照片，覆盖黑白层次预设、1448×1086 PNG 完整显示、独立重开、换任务和旧下载清理。原有两条旅程继续保留既有 Chrome / Edge 2/2 证据；第三条必须由新的浏览器报告闭合，不能倒填为历史证据。
...
“基础功能冻结”不等于产品功能完成。当前可执行的场景技能层包含商品白底图、报名照 / 底色头像、社交头像与封面、社交九宫格切图、老照片基础整理。前两项编排 Provider、蒙版与下载合同；后三项可以完全本地运行。`UT-OLD-PHOTO` 只将公开光色预设、构图和导出组织成基础整理，不声称去划痕、修脸、降噪或补全。`UT-GRID` 只做方形构图、九格拆分和本地交付，不识别主体、不补画或自动发布。`CR-RESTORE` 继续作为分离的 `gpt-image-2` 生成式实验，使用有限提示词合同、风险确认和独立命名；它尚无真实质量结果，不得称为无损、档案级或身份保真修复。主体感知布局和正式证件规格仍必须另立能力。

场景层增量：`UT-PORTRAIT` 结果页现先提供与修边分离的最终报名照预览。用户可选通用方形 `600 × 600`、通用竖版 `480 × 600` 或保留当前像素，并按 Alpha 非透明边界手动控制人物占比和上下位置；背景继续使用白、黑、彩色或自定义纯色。该路径不做自动人脸/头顶定位，预览与下载使用同一绘制函数。当前底色头像还可确定性排成 `1800 × 1200`、3 × 2 的六宫格 JPEG；该步骤不再调用 Provider，不改写蒙版，并在本地验证容器、metadata 和重开尺寸。它只解决普通像素构图与“同一头像多张排版”的交付需求；DPI、毫米尺寸、地区 / 机构规则与官方受理继续不在承诺范围。

`UT-PORTRAIT` 同时增加通用“一照多用”套装：当前抠图、修边和人物构图同步派生 `600 × 600` / `480 × 600` × 白 / 蓝 / 暖红 6 个 JPEG。六个卡片预览、主预览切换、单张下载与整组 ZIP 都只在本机重绘；每个下载仍检查容器、metadata、尺寸、像素、哈希和长度，并以 busy lock 与 run/result identity 阻止重复操作和旧结果回填。像素与颜色只是技术起点，不新增自动人脸分析、官方规格或机构受理声明。

`UT-PRODUCT` 的交付层现增加本地商品大小、双轴留白、柔和阴影和通用像素模板：修边画布保持原坐标系，避免布局缩放破坏画笔命中；大小和位置按 Alpha 中全部非透明像素的包围盒等比计算，避免透明画布留白扭曲用户看到的主体占比；这不是商品识别，也不会识别或删除水印。同一套构图会同步派生 `1200 × 1200` 方形、`1200 × 1500` 竖版、`1200 × 900` 横版和当前像素四个白底 JPEG，四卡片同时预览，可单独下载，也可在全部 metadata / 尺寸 / 像素重开检查通过后写入单个本地 ZIP。该操作不重复调用 Provider，50 MiB 上限和并发忙碌锁会阻止异常整组交付。模板只表达像素画布，不做接触面估计、光照理解、平台规则或多来源图片批处理。
...
`UT-TEMPLATE` 现已把常用比例和导出上限组织为独立本地任务：1:1、4:5、16:9、9:16 均进入同一 normalized crop 与 renderer 合同，五个预设只组合比例和最长边上限；小图不放大，实际尺寸单独显示。它另提供一个可选短标题：最多 40 字、最多两行，支持上下位置、左对齐 / 居中、浅色 / 深色，并以通用 7% 安全区和可读底板写入真实导出像素。当前结果页会复用同一构图和标题，确定性重绘方形分享、竖版分享、横版封面和竖屏故事四个文件；四卡预览、主预览切换、单张下载与整组 ZIP 都在本机完成，每个文件先核对格式、尺寸、像素、哈希和长度，任务变化时旧异步输出不能进入页面。当前仍不做主体感知裁切、扩图、贴纸、字体库、多图层编辑、平台规则同步或发布检查，因此不得把按钮名称或安全区解释为平台合规证明。内部浏览器诊断已覆盖四规格的真实 Canvas 编码、标题像素、重开和 ZIP；同源自驱 Chromium 路径此前也已覆盖单张标题预览、QA、Blob 捕获与 `1440 × 810` PNG 重开。原生鼠标 / 键盘、四卡实际观感与操作系统下载目录仍需现场观察，不再在这一窄标题能力上扩小控件。
...
已完成场景增量：老照片基础整理会在同一几何与格式下生成四个本地光色版本，并提供比较、单张下载与 ZIP。其完成条件只覆盖确定性光色整理和交付完整性，不覆盖生成式修复质量。
...
8. ~~建立 `UT-OLD-PHOTO` 本地老照片基础整理，并把 `CR-RESTORE` 保留为分离的 AI 实验：本地路径无需 API；生成式路径继续使用有限设置、风险确认和副本下载，不做未经授权的计费调用或质量声明；~~
9. 继续界面收敛并验证第三条本地浏览器旅程；只有用户明确选择供应商、配置独立 API 凭据并授权单次预算后，才用项目原创或公版旧照片夹具运行生成式质量检查。MiniMax / OpenAI 都不是本地基础整理的前置条件；
...
当前下一次工作以[产品能力与工程演进执行计划](PRODUCT_AND_ENGINEERING_EXECUTION_PLAN.md)为准，并继续服从[产品稳定化执行路线的近期实施包](PRODUCT_STABILIZATION_ROADMAP.md#9-近期实施包)。`UT-SOLID-BG` 已从产品顶层 catalog、runtime profile 和下载合同移除，纯色换底继续作为透明抠图 / 商品 / 报名照结果工作台中的派生交付动作。Stage 0 现在只剩创建工作树 checkpoint；随后进入 Stage 1 的 `source-task-controller` / `settings-controller` 无行为变化抽离。稳定 build 形成后执行 2–3 人走查和代表性图片矩阵；区域理解、自动构图和真正修复按 Stage 4–6 进入，不直接堆入当前 `main.js`。
