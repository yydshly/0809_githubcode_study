...
产品入口现增加一层可执行的[场景技能](SCENARIO_SKILLS.md)：优先呈现“商品白底图”“报名照 / 底色头像”“社交头像与封面”，再显示自然增强、保真整理和透明抠图等自由工具。场景技能不是新的模型，而是把已有构图、抠图、修边、换底和导出合同组织成普通用户能理解的用途。商品与报名照场景依赖已配置的远程抠图；社交构图完全在本机完成。老照片修复仍未实现，不会用普通调色冒充划痕修复、面部恢复或生成式补全。
...
基础链路可用不等于所有场景都完成。**商品白底图**现会载入 1:1、1600 px 上限和白底起点，经现有 Provider 抠图后继续本地修边，并只允许不透明 JPEG 下载；它不生成阴影，也不保证平台审核。**报名照 / 底色头像**会载入人物略上移的构图起点，支持 1:1 / 4:5 与本地选底，但不承诺护照、签证或机构受理。**社交头像与封面**复用完全本地的比例与尺寸模板。创意生成已有真实 API 路径但当前服务未配置；自然增强仍只是公开的固定光色参数。批量、主体感知裁切、扩图、正式证件规格和老照片修复均未实现。
