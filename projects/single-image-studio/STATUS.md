...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 10 开放 calibration 范围已冻结；协议、definition preview 与 durable runner foundation 均已 fake-tested | `Slice 10: scope-frozen / protocol-foundation-fake-tested / definition-preview-fake-tested / runner-foundation-fake-tested / registered-driver-not-created / definition-not-created / calibration-not-run / formal-holdout-not-created / non-C1 / non-product` | runner 7/7：144-slot pass/non-pass、432-event ledger、零 retry、protocol/global stop、runtime-end drift hard-stop；尚未接 actual adapter，正式 definition 提交推送前禁止 calibration |
...
| Slice 10 范围、协议、definition preview 与 runner foundation | scope 合同已冻结；协议 7/7、preview 9/9、runner 7/7 | normalize / export 各 48 sources × 3 = 144 attempts；合计 96 / 288；临时 fake closure 证明零 retry 与完整 ledger | 只允许项目原创 public-synthetic lineage；不复制 canonical 图片；registered driver、actual adapter 接线、正式 definition / request / result 与 formal holdout 均未创建。详见 [Slice 10 合同](research/SLICE_10_CONTRACT.md) |
...
12. **Slice 10 scope、协议、机器定义预览与 durable runner foundation 已 fake-tested**：按 [Slice 10 合同](research/SLICE_10_CONTRACT.md) 计划 normalize / export 各 30 `dev/calibration` + 18 `defect/calibration` source、每来源 3 次，共 96 source / 288 attempts；固定零 retry / replacement。协议 7/7、definition preview 9/9；两个固定 UTC 临时构建逐字节一致，形成 16 strict schemas、96 个新 source identity、48 个 gold identity 与 4 份 manifest。runner foundation 另以 7/7 系统临时测试闭合每 operation 48×3、432-event hash-chain ledger、原子 pass publication、普通 non-pass 不缩分母、protocol/inconclusive 全局停止和 runtime-end drift 无 summary；stage 中 publication 只能声明 prepared，commit 由 rename 后 ledger event 证明。它尚未接 actual adapter / registered driver，definition preview 也须吸收最终 runner hash 后重新生成；正式 definition、results 与 formal holdout 仍未创建。全量 verify 为 450/450。
