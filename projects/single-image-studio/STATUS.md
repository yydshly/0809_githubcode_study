...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 10 已 fail closed；Slice 11 Phase B foundation 已实现 | `Slice 11: scope-frozen / projection-and-lifecycle-foundation / definition-not-created / calibration-not-run / non-C1 / non-product` | exact 12-field projection 与 versioned lifecycle adapter 通过 7 / 7 fake-only tests；真实 calibration 仍须等待完整协议与新 results-zero definition 提交推送 |
...
| Slice 11 foundation | `@0.11.0`；projection / adapter 已实现，definition / results 未创建 | 新身份的 96 sources / 288 attempts；零 retry / replacement | 投影保留完整 gold hash并只下发 12 keys；preflight / spawn / IPC / exit 生命周期按事实派生；7 / 7 fake-only。详见 [Slice 11 合同](research/SLICE_11_CONTRACT.md) |
...
13. **Slice 11 projection / lifecycle foundation 已实现，definition 尚未创建**：`@0.11.0` 的 content-addressed projector 保留完整 gold canonical JSON/hash，只把 exact 12 fields交给 adapter；versioned adapter 独立记录 preflight、spawn、IPC 与 exit，7 / 7 fake-only tests 覆盖缺字段、自重哈希洗白、pre-worker、spawn failure、clean exit 与 worker-declared failure。下一步仍需完整 case / runner / schema / central definition，冻结并推送 results-zero baseline 后才可能运行；它仍不是 C1 或产品支持。
