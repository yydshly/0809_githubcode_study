...
| 维度 | 当前结论 |
| --- | --- |
| 研究阶段 | Slice 01–10 保持各自不可改写的历史边界。Slice 10 唯一 calibration 在 1 / 288 后 protocol-failed；Slice 11 protocol + durable attempt foundation 已通过 33 / 33 fake-only tests，machine definition / results 尚未创建。C1 与产品支持仍为 0 |
...
[Slice 11 合同](research/SLICE_11_CONTRACT.md) 建立 `@0.11.0` 新身份，只修 gold-to-adapter projection 与真实 worker lifecycle。projection、versioned adapter、case executor、strict lifecycle record，以及 versioned request / terminal / ledger / summary runner 已实现。durable attempt bridge 先持久 request / claim，再为 applicable、rejection 或 failure 原子发布不同的严格 closure；只有发布完成才会进入下一 slot，post-rename 不确定性必须全局停止。33 / 33 fake-only tests 包含完整 48×3 临时文件树，不调用 Sharp。canonical 96-source / 288-attempt machine definition、request、result仍未创建；durable event ledger / operation close、central validator 和 results-zero definition 分别提交推送前，禁止真实 calibration。
