...
Slice 10 当前新增本地原创 scope 合同、workspace README、只依赖 Node builtins 的 calibration protocol foundation，以及不提供 materialization CLI 的 definition-preview generator。协议 fake-only tests 在内存构造严格 admission / request / terminal / summary；definition tests 只读取 Sharp 的版本 inventory 和仓库内既有项目原创 Slice 05 open-synthetic records，在两个系统临时树构造 176 个 JSON/schema 文件并逐字节比较，不调用 Sharp 图像管线、不复制或生成图片 bytes、不写 canonical definition/results，也不把 Slice 05–09 的 request / result 拼入新分母。外部 lineage 均按实际 record/file SHA 绑定，Slice 09 仅以双 Gate-B pass admission 引用。当前没有新增 dependency、runtime artifact、第三方代码或资产、真实图片、模型、权重、calibration result、formal holdout 或产品接线；任何 Sharp calibration 必须等 durable runner 和 results-zero definition 提交并推送后才可运行。
