...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 10 开放 calibration 范围已冻结；协议、definition preview、durable runner 与 actual-case adapter foundation 均已 fake-tested | `Slice 10: scope-frozen / protocol-foundation-fake-tested / definition-preview-fake-tested / runner-foundation-fake-tested / actual-case-adapter-fake-tested / registered-driver-not-created / definition-not-created / calibration-not-run / formal-holdout-not-created / non-C1 / non-product` | case adapter 9/9：重开 96 个 source lineage、48 个 independent gold、全部 48 个 rejection worker-free；registered driver 与正式 definition 尚未创建，提交推送前禁止 calibration |
...
| Slice 10 范围、协议、definition preview、runner 与 case adapter foundation | scope 合同已冻结；协议 7/7、preview 9/9、runner 7/7、case adapter 9/9 | normalize / export 各 48 sources × 3 = 144 attempts；合计 96 / 288；临时 fake closure 证明零 retry 与完整 ledger | actual-case adapter 已按真实 Slice 05 bytes / record / gold lineage 全量重开，但只注入 fake raw executor；registered driver、正式 definition / request / result 与 formal holdout 均未创建。详见 [Slice 10 合同](research/SLICE_10_CONTRACT.md) |
...
12. **Slice 10 scope、协议、机器定义预览、durable runner 与 actual-case adapter foundation 已 fake-tested**：按 [Slice 10 合同](research/SLICE_10_CONTRACT.md) 计划 normalize / export 各 30 `dev/calibration` + 18 `defect/calibration` source、每来源 3 次，共 96 source / 288 attempts；固定零 retry / replacement。协议 7/7、definition preview 9/9；两个固定 UTC 临时构建逐字节一致，形成 16 strict schemas、96 个新 source identity、48 个 gold identity 与 4 份 manifest。runner foundation 以 7/7 系统临时测试闭合每 operation 48×3、432-event hash-chain ledger、原子 pass publication、普通 non-pass 不缩分母、protocol/inconclusive 全局停止和 runtime-end drift 无 summary。actual-case adapter 再以 9/9 fake-only tests 重开全部 96 个 Slice 05 source lineage 与 48 个 independent gold identity；48 个 rejection 均在 worker 前按真实 bytes / record 精确分类，applicable 路径只注入 fake raw executor，并由独立 PNG decoder 复核输出。registered driver 仍未创建，definition preview 也须吸收最终 runner / driver hash 后重新生成；正式 definition、results 与 formal holdout 仍未创建。
