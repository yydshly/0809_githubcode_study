...
表中的“首批研究重点”是待验证范围，不是当前产品格式或能力清单。`CAP-02` 的 Slice 05–08 失败历史保持不可改写；Slice 09 的 canonical PNG normalize / export 唯一 open smoke 已双 Gate-B pass。Slice 10 的 96-source / 288-attempt 开放 calibration definition 已冻结，但唯一 registered invocation 在首个 normalize attempt 因 expected-object shape 不兼容于 Sharp worker 前停止；export 未启动，且 terminal lifecycle 记录失真，central fail closed。Slice 10 不得重跑，任何修复必须进入 Slice 11 新定义。即使未来 calibration 通过，也不是 formal holdout、C1 或产品支持。JPEG / WebP 等行继续 fail closed，全部格式仍为 `productSupport=false`。
