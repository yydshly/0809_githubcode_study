...
| `REG-NORM-SHARP-CANONICAL-PNG@0.10.0` | Slice 09 像素 / encoder lineage + 新 open-calibration protocol identity | `CAP-02` | canonical PNG normalize / export | `planned/protocol-preview-runner-foundation-fake-tested` | 计划 normalize/export 各 48 sources × 3，共 96/288；零 retry/replacement；协议 7/7、definition preview 9/9、durable runner foundation 7/7。registered actual driver、正式 definition、results 与 formal holdout 均 not-created；非 C1、非产品支持 |
...
8. Slice 11 / `@0.11.0` scope 已冻结；content-addressed gold-to-adapter exact 12-field projector 与事实派生的 lifecycle adapter foundation 已通过 7 / 7 fake-only tests。它不调用 Sharp，并能区分 preflight-not-started、spawn-attempted、IPC 与 confirmed exit。case / runner / machine definition 与 results 仍未创建；完整实现和 results-zero 基线分别提交推送前不得运行。
