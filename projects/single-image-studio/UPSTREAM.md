...
Slice 08 Phase A / B 只新增本地原创 typed case-context 范围与协议模块。fake-only tests 使用内存 records / functions，不新增 dependency、upstream package、第三方代码 / 图片 / 模型 / 权重，也未运行 Sharp 图片路径或生成 canonical result。候选像素与 encoder 架构继续精确引用 Slice 07；后续 versioned protocol 仍只能使用 Node builtins 与现有已锁定 research-only runtime，任何依赖或 native drift 都要求停止并重新冻结。
