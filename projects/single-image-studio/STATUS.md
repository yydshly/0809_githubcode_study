...
M2 的 [基础编辑器早期内部走查卡](M2_INTERNAL_WALKTHROUGH.md) 已就绪，包含固定 2 分钟任务、同意话术、主持规则、匿名记录字段和 80% 通过算法；真实参与者场次尚未开始，当前分母为 `0/5–8`。材料就绪不是走查通过，也不提升任何证据轴。
...
结果页现可直接“换个处理方向”：保留已经确认的当前图片和任务资格，清除旧结果、旧下载资格、旧任务配置与活动 run，再聚焦第一个可用任务。设置页和错误页返回任务列表也复用同一状态迁移，不再只切换可见页面而把旧任务留在后台；该导航不会上传图片或自动开始新任务。自驱浏览器验收页现会在 1280×720 与 1440×900 两条本地真实流程中继续核对完整显示、横向溢出、结果换任务、设置返回、焦点与旧下载失效；它给当前浏览器会话提供可见结论，但仍不等于 Chrome / Edge 两个正式版本矩阵已经完成。
...
这次冻结只针对已经够用的基础链路，不代表页面上的全部大模块都已完成。`CR1` 创意生成已有真实远程执行路径，但当前运行服务报告未配置；`UT-PORTRAIT` 原先确实只有不可选占位卡，现已改为可运行的“通用底色头像”：在本机以完整预览确定 1:1 / 4:5 构图、方向、光色和工作分辨率，逐次确认后只把该构图发送给现有抠图 Provider，返回后继续使用本地修边、白 / 黑 / 彩色 / 自定义底和不透明 JPEG 下载。它不承诺护照、签证或任何机构规格；沙盒水印、正式质量、真实浏览器 E2E 与许可头像任务验收仍是欠项。

页面现新增独立的 `UT-ENHANCE`“自然增强”本地任务。它复用同一 renderer、history、完整预览、比较、输出重开和下载合同，提供五个固定且公开的克制预设：自然提亮、清亮、鲜明、柔和、原始光色；每个预设只写入用户可见的亮度 / 对比度 / 饱和度，手动修改后不会被自动覆盖。该任务不上传图片、不识别人像 / 天空、不自动判断照片质量，也不进行降噪、白平衡诊断、局部增强、清晰度恢复或生成式补细节。因此它是内部试用的**手动可解释增强路径**，不是 `effect.natural-enhance` 的自动效果质量证明，U1 / C1 仍为 0。

页面现新增独立的 `UT-TEMPLATE`“场景尺寸模板”本地任务。它在同一编辑合同中提供方形分享 1:1 / 1080、竖版分享 4:5 / 1350、横版封面 16:9 / 1920、竖屏故事 9:16 / 1920、商品方图 1:1 / 1600 五个透明预设；数值表示最长边上限，小图不会被放大，实际输出尺寸始终在左侧预览旁显示。模板切换会真实改变亮框、渲染尺寸与下载结果，用户仍可调整保留位置、旋转、光色和格式；手动改动比例或尺寸后模板高亮会取消。它不是任何平台的永久官方规范，不包含主体感知裁切、扩图、文字排版或发布校验，也不改变 C1 / U1 / 产品发布状态。

内部浏览器诊断现进一步用内存合成图调用与产品相同的 renderer，真实生成并重开 16:9 与 9:16 场景模板输出，核对裁切尺寸、像素可重开和“小图不放大”。该检查不读取用户图片、不访问网络，也不替代主产品页的鼠标、键盘、长宽图拖动和 Chrome / Edge 版本矩阵验收。

任务选择页现按处理边界分成“先在本机完成”“主体与背景”“创意生成”三组，并逐组显示可用数量。本地操作、需要逐次确认上传的抠图操作，以及尚未配置的创意服务不再混排；这只改善信息层级，不会把不可用服务变成可用，也不会根据图片内容自动推荐任务。
