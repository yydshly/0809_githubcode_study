...
| Slice 06 诊断协议 | Phase B fake-only 实现已就绪；canonical definition 与 results 为 `not-created` | 4 scripts；3 个落盘 oracle record schemas + 10 个 runner-exported strict schema documents；3 tests；fake-only `42 / 42`；full verify `270 / 270` | adapter / worker / independent diagnostic oracle / durable local runner 已实现并通过协议负例；测试只在内存构造 synthetic PNG bytes、在系统临时目录构造 fake closure，未运行真实 Sharp 图片路径。candidate / contracts / plans / preregistrations / manifests / runtime / hardware / fixture wrappers / definition index / results 均仍为 `not-created`。本切片没有 Gate B decision authority，不创建 calibration、artifact、正式 holdout 或产品接线 |
...
8. **已完成 Slice 06 Phase B 诊断协议实现**：在 [Slice 06 合同](research/SLICE_06_CONTRACT.md) 的窄边界内新增 isolated worker、strict adapter、独立 PNG diagnostic oracle、durable runner、3 份落盘 record schema 与 runner 导出的 10 份 strict schema document；fake-only oracle / runner / runtime 合计 `42 / 42`，独立审计 `P1 / P2 / P3 = 0 / 0 / 0`。这些测试没有运行真实 Sharp 图片路径；新 `REG-NORM-SHARP@0.6.0`、两份 `CapabilityContract@0.6.0`、两份 preregistration、8 个 source wrapper、fresh runtime / hardware 与 definition index 仍为 `not-created`。
