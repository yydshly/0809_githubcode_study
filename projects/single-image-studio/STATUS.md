...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 08 typed case-context 范围已冻结 | `Slice 08: scope-frozen / implementation-not-started / definition-not-created / Gate-B-smoke-not-run / calibration-forbidden / non-C1 / non-product` | Slice 07 保持 closed non-pass；Slice 08 将以新版本完整执行 12 sources / 36 attempts，不拼接或补跑旧结果 |
...
| Slice 08 范围 | typed runner → driver case-context 已冻结，尚未实现 | 计划 12 sources / 36 attempts；新 `@0.8.0` IDs；results=not-created | 只修 disposition / expected-code / source / manifest 的 closed hash-bound 传递；candidate pixels / encoder / oracle profile 不变；真实 Sharp 路径禁止到 results-zero definition 提交并推送之后 |
...
10. **Slice 08 scope-only 已冻结**：按 [Slice 08 范围合同](research/SLICE_08_CONTRACT.md) 新建 typed `caseContext` 协议；不是只补跑 18 个 rejection，而是在新版本、新 ID、新 request 下完整计划 normalize / export 各 18 次。当前 implementation、definition、result、decision 均未创建，calibration 继续禁止。
