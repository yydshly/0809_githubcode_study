...
9. **Slice 07 scope 已冻结，不得重跑 Slice 06**：新版本选择 Sharp 解码 / 像素处理 + candidate-owned canonical PNG encoder，且生产 encoder 必须与 oracle 实现隔离。计划先实现 fake-tested candidate，再冻结 results-zero definition；只有定义基线提交并推送后才允许一次 12-source / 36-attempt registered Gate-B smoke。当前 Gate B、calibration、C1、产品支持继续为 0 / false。
