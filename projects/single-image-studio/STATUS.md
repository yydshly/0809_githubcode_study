...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 10 开放 calibration 范围已冻结；fake-only 协议基础已实现 | `Slice 10: scope-frozen / protocol-foundation-fake-tested / runner-not-created / definition-not-created / calibration-not-run / formal-holdout-not-created / non-C1 / non-product` | admission / request / terminal / 48×3 summary 四份严格记录与 7/7 对抗测试已建立；Slice 09 只作不可变 lineage，96 / 288 definition 提交推送前禁止 calibration |
...
| Slice 10 范围与协议 | scope-only 合同已冻结；协议基础 fake-tested | normalize / export 各 48 sources × 3 = 144 attempts；合计 96 / 288；协议测试 7/7；results=0 | 只允许项目原创 public-synthetic lineage；零 retry / replacement；durable runner、definition、runtime observation、request、result 与 formal holdout 均未创建。详见 [Slice 10 合同](research/SLICE_10_CONTRACT.md) |
...
12. **Slice 10 scope 已冻结，协议基础已 fake-tested**：按 [Slice 10 合同](research/SLICE_10_CONTRACT.md) 计划 normalize / export 各 30 `dev/calibration` + 18 `defect/calibration` source、每来源 3 次，共 96 source / 288 attempts；固定零 retry / replacement。四份严格协议记录覆盖 admission、request、terminal 与 48×3 summary，7/7 fake-only tests 已通过；它们拒绝 attempt 2、缺失 / 重复分母、分区 / operation 串线、generic rejection 和多数洗白。durable runner、definition、runtime、results 与 formal holdout 仍未创建；必须先完成 tooling 和 results-zero definition freeze / push，才可运行唯一 registered calibration。
