...
- 当前产品自动测试为 `16 files / 108 tests`，动态语法检查覆盖 `167` 个脚本。工作区会实时显示构图位置、实际输出尺寸与格式，支持拖动、方向键、滑杆、自定义宽高、撤销、重做和重置；响应式静态合同还覆盖 980px 单列、390px 相邻规则、可见焦点和 reduced-motion。上述结论目前只有代码与自动测试支持。
