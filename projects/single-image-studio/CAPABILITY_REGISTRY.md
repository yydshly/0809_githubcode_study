...
| `REG-NORM-SHARP-CANONICAL-PNG@0.11.0` | Slice 10 immutable failure lineage + versioned expected projection / truthful lifecycle | `CAP-02` | canonical PNG normalize / export | `scope-frozen/fake-only-protocol-and-durable-foundation` | projection / case / lifecycle / protocol runner 与 durable claim / runtime / atomic publication primitives 通过 27 / 27 fake-only tests；registered driver、96/288 definition 与 canonical results 尚未创建；零 retry/replacement，非 C1、非产品支持 |
...
8. Slice 11 / `@0.11.0` scope 已冻结；projection、事实派生 lifecycle、frozen-material case executor、versioned request / terminal / ledger / summary runner，以及 durable claim / runtime observation / oracle facts / atomic publication primitives 已通过 27 / 27 fake-only tests。它不调用 Sharp；claim 对重放不授予二次执行权，applicable closure 只在 intent 后原子 rename，post-rename 异常保留唯一 closure 并进入 reconciliation-unknown。registered durable driver、machine definition 与 canonical results 仍未创建；完整实现和 results-zero 基线分别提交推送前不得运行。
