...
| `REG-NORM-SHARP-CANONICAL-PNG` | Sharp RGBA8 pixel stage + 项目原创 canonical PNG encoder | `CAP-02` | canonical PNG normalize / export | `implementation-in-progress`（非 Gate B） | encoder 已通过内存 independent-oracle tests `5/5`；Sharp raw worker、adapter、runner、definition / Gate-B result 未创建，两项 Gate B 全过前 calibration 禁止 |
...
| `CC-CAP02-NORMALIZE-PNG@0.7.0` | `CAP-02` | canonical PNG source → future `NormalizedImage` v0.7 binding | planned `REG-NORM-SHARP-CANONICAL-PNG@0.7.0` | scope-only：Sharp 输出 RGBA8，candidate-owned encoder 写固定 closed PNG profile；3 applicable + 3 rejection × 3，implementation / definition / Gate-B result 均未创建，C1=0 |
| `CC-CAP02-EXPORT-PNG@0.7.0` | `CAP-02` | independent normalized input → future `DeliveryArtifact` v0.7 binding | planned `REG-NORM-SHARP-CANONICAL-PNG@0.7.0` | scope-only：与 normalize 共用候选架构但独立 decision；3 applicable + 3 rejection × 3，两项都 pass 前 calibration 禁止，C1=0 |
...
4. Slice 07 scope 已选择 `REG-NORM-SHARP-CANONICAL-PNG@0.7.0`：Sharp 只产出 RGBA8，项目原创 encoder 写 closed PNG profile，independent oracle 只验收。当前 implementation / definition / result 为 0；先提交并推送实现与 results-zero definition，才允许一次 36-attempt registered smoke。
