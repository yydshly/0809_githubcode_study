...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 10 开放 calibration 范围已冻结；协议、preview、runner、case adapter、registered driver 与 runtime-end observer 均已 fake-tested | `Slice 10: scope-frozen / runner-foundation-fake-tested / actual-case-adapter-fake-tested / registered-driver-foundation-fake-tested / durable-runtime-end-observer-fake-tested / central-validator-not-created / definition-not-created / calibration-not-run / non-C1 / non-product` | observer 6/6：144 个 terminal 后重新盘点 runtime、漂移与重放均关闭失败；尚不可执行，正式 definition 提交推送前禁止 calibration |
...
| Slice 10 范围、协议、preview、runner、case adapter、registered driver 与 runtime-end observer | scope 合同已冻结；协议 7/7、preview 9/9、runner 7/7、case adapter 9/9、driver 5/5、observer 6/6 | normalize / export 各 48 sources × 3 = 144 attempts；合计 96 / 288 | observer 在全部 terminal 后只读重盘并原子写入 distinct runtime-end record；尚缺 central validator 与正式 definition，result 与 formal holdout 均未创建。详见 [Slice 10 合同](research/SLICE_10_CONTRACT.md) |
...
12. **Slice 10 scope、协议、机器定义预览、durable runner、actual-case adapter、registered driver 与 runtime-end observer 已 fake-tested**：按 [Slice 10 合同](research/SLICE_10_CONTRACT.md) 计划 normalize / export 各 48 source × 3，共 96 source / 288 attempts；固定零 retry / replacement。协议 7/7、preview 9/9、runner 7/7、case adapter 9/9、driver 5/5、observer 6/6。preview 现包含 22 份 strict schemas，并逐字节 pin runner、case adapter、driver 与 observer 实现。case adapter 重开全部 96 个 Slice 05 source lineage 与 48 个 independent gold；48 个 rejection 均在 worker 前精确分类。registered driver 证明 central / Git / results-zero admission、单 invocation 顺序两项 operation、普通 complete non-pass 继续另一项，以及 protocol uncertainty 全局停止。runtime-end observer 只在全部 terminal 后重新盘点 package / native / runtime，逐字节比对冻结 start attestation，并以 `wx` 与 fsync 发布 distinct record；漂移、非法 UTC、嵌入 payload 伪造与重放均关闭失败，且不处理图片。central validator 尚未创建，driver 因而仍不可执行；正式 definition、results 与 formal holdout仍未创建。
