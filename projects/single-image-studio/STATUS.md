...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 10 已 fail closed；Slice 11 Phase B protocol foundation 已实现 | `Slice 11: scope-frozen / projection-case-lifecycle-and-in-memory-runner-foundation / definition-not-created / calibration-not-run / non-C1 / non-product` | projection、case、lifecycle、request / terminal / ledger / summary 与纯内存 48×3 runner 通过 20 / 20 fake-only tests；真实 calibration 仍须等待 durable publication、central validator 与新 results-zero definition 提交推送 |
...
| Slice 11 foundation | `@0.11.0`；projection / adapter / case / lifecycle / pure protocol runner 已实现，definition / results 未创建 | 新身份的 96 sources / 288 attempts；零 retry / replacement | 6 strict schemas；terminal 引用独立 lifecycle / projection；ledger 两事件成对哈希链；summary 固定每 operation 48×3 且禁止 majority；20 / 20 fake-only。详见 [Slice 11 合同](research/SLICE_11_CONTRACT.md) |
...
13. **Slice 11 projection / case / lifecycle / protocol-runner foundation 已实现，definition 尚未创建**：`@0.11.0` 的 projector、adapter 与 case executor 保留完整 gold identity并只向底层下发 exact 12 fields；strict lifecycle record 独立记录 preflight、spawn、IPC 与 exit。新增纯内存 runner 生成 versioned request / terminal / ledger / summary，固定每 operation 48×3、零 replacement；terminal 交叉绑定 lifecycle / projection，ledger 强制 start-terminal 成对链，summary 从 144 terminals派生 pass 与 byte determinism。20 / 20 fake-only tests覆盖重哈希、串线、分母、majority 与全局停止。下一步仍需 durable claim / publication / runtime-end、central validator 与 machine definition；冻结并推送 results-zero baseline 后才可能运行。它仍不是 C1 或产品支持。
