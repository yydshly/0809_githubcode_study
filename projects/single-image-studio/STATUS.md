...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 10 开放 calibration 范围已冻结；协议与机器定义预览均已 fake-tested | `Slice 10: scope-frozen / protocol-foundation-fake-tested / definition-preview-fake-tested / runner-not-created / definition-not-created / calibration-not-run / formal-holdout-not-created / non-C1 / non-product` | 四份运行协议 schema 加十二份 definition schema；176-file 双临时预览逐字节一致，96 source wrappers / 48 gold identities / 4 manifests，正式 definition 提交推送前禁止 calibration |
...
| Slice 10 范围、协议与 definition preview | scope 合同已冻结；协议 7/7、definition preview 9/9 | normalize / export 各 48 sources × 3 = 144 attempts；合计 96 / 288；16 strict schemas；临时预览 176 files | 只允许项目原创 public-synthetic lineage；不复制图片；零 retry / replacement；runner、正式 definition、request、result 与 formal holdout 均未创建。详见 [Slice 10 合同](research/SLICE_10_CONTRACT.md) |
...
12. **Slice 10 scope、协议与机器定义预览已 fake-tested**：按 [Slice 10 合同](research/SLICE_10_CONTRACT.md) 计划 normalize / export 各 30 `dev/calibration` + 18 `defect/calibration` source、每来源 3 次，共 96 source / 288 attempts；固定零 retry / replacement。四份严格运行协议记录 7/7，definition preview 9/9；两个固定 UTC 临时构建逐字节一致，形成 16 strict schemas、96 个新 source identity、48 个 gold identity 与 4 份 manifest，并按实际 bytes 绑定 Slice 05 开放 synthetic lineage 和 Slice 09 双 Gate-B pass。预览不复制图片且没有 materialize 到仓库；durable runner、正式 definition、results 与 formal holdout 仍未创建。全量 verify 为 443/443；必须先完成 runner、中央 validator 和 results-zero definition freeze / push，才可运行唯一 registered calibration。
