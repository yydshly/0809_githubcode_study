...
表中的“首批研究重点”是待验证范围，不是当前产品格式或能力清单。`CAP-02` 的 Slice 05–08 失败历史保持不可改写；Slice 09 的 canonical PNG normalize / export 唯一 open smoke 已双 Gate-B pass。Slice 10 的唯一 calibration 在首个 normalize attempt 因 expected-object shape 不兼容于 Sharp worker 前停止，且 terminal lifecycle 记录失真；同版本不得重跑。Slice 11 的 versioned protocol + durable foundation 已通过 27 / 27 fake-only tests，包括幂等 claim、runtime observation、六文件原子 closure 与 reconciliation fail-closed；registered durable driver、central definition 与 canonical results 仍未创建。即使未来 calibration 通过，也不是 formal holdout、C1 或产品支持。JPEG / WebP 等行继续 fail closed，全部格式仍为 `productSupport=false`。
