...
| `REG-NORM-SHARP-CANONICAL-PNG@0.9.0` | 相同像素 / encoder 架构 + versioned gold identity | `CAP-02` | canonical PNG normalize / export | `definition-frozen/results-zero` | definition commit `36d92844a2ea58113567a24482e5297ba8cdd9ab`；18 schemas / 12 sources / 6 identities / 36 planned attempts / copied bytes 0 / results 0，full tree `a1c06dc0…bc6b1`；registered smoke 未运行，非 Gate B |
...
| `CC-CAP02-NORMALIZE-PNG@0.9.0` | `CAP-02` | canonical PNG source → future normalized artifact binding | frozen definition `REG-NORM-SHARP-CANONICAL-PNG@0.9.0` | contract `contentHash=9788a9cb9c9c539f502a2f01695be3f74d0a70868b44f6807561a8eb39d3fee5`；完整 18-attempt operation 分母、gold identity 与 stop rule 已冻结，results=0、calibration=false、C1=0 |
| `CC-CAP02-EXPORT-PNG@0.9.0` | `CAP-02` | independent normalized input → future delivery artifact binding | frozen definition `REG-NORM-SHARP-CANONICAL-PNG@0.9.0` | contract `contentHash=4bfd1de8baa2d312f68e208a1dc982b45e05c6ee252dbdfbc76085765544794e`；不得重用 candidate normalize output，完整 18-attempt operation 分母已冻结，results=0、calibration=false、C1=0 |
...
6. Slice 09 results-zero definition 已以 commit `36d92844a2ea58113567a24482e5297ba8cdd9ab` 推送；当前 results=0。只允许 fresh validator、clean/pushed Git admission 与 absent result root 全部通过后执行一次 registered smoke，不得选择性补跑。
7. calibration 后的最终版本通过独立预注册审计，才由 custodian 建立外部 bundle、完成 external pins / isolation audit、签发具体一次性 request 并运行；Slice 04 的 seal intent 不能替代该 request。
8. standalone `REG-NORM-LIBVIPS` 若要成为另一个候选，必须另行锁定 artifact、构建选项、格式库与 LGPL 交付方式；不能把 Sharp bundled libvips 重复计算为第二臂。
9. Matting、自然增强和创意候选保持后置；将 checkpoint、依赖和训练数据结论逐项写回本表，无法消除的商用限制转为 `no-go`。独立 holdout、defect 与重复性证据齐全后才评审 C1。
