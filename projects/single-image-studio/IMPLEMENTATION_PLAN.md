...
禁止重跑 `@0.5.0`–`@0.11.0` 或补跑旧 calibration。Slice 11 的两文件 startup failure 已封闭；不再创建 Slice 12。项目已回到原始能力链并完成第一条可运行纵向 baseline：3 个原创 `MATTE-GT dev/calibration` 场景经过严格 sRGB normalization、SourceCard.v0、fixture-known SubjectMap 与 MATTE-SIMPLE。hard-edge / hole MAE=0；soft-edge MAE=6.9314、IoU@128=0.92227，明确证明 simple baseline 只能作比较下限。`SCO-EXPOSURE-SIGNALS@0.1.0` 已提供客观亮度分布，语义 exposure 仍 unknown；候选中立 continuous-alpha、自然人像/模型获取治理和候选运行时元数据定义均已冻结。`DEF-MATTE-RUNTIME-METADATA@0.1.0` 登记 10 份固定提交官方文本：MODNet 只有官方 Drive 文件夹而无 immutable object/hash；RVM 有固定 release URL 与 exact direct requirements，但 Python/platform wheel/transitive hash/SBOM 未闭合。safe loader 未实现，HEAD/GET 模板未签发。实际自然图片、模型、候选 runtime、依赖和 results 仍全为 0。下一步只能独立评审 exact Python/platform wheel/SBOM proposal，随后至多签发 metadata-only HEAD；model body GET 继续单独禁止。
