...
这次冻结只针对已经够用的基础链路，不代表所有图片场景都已完成。任务选择现增加场景技能层：`UT-PRODUCT`“商品白底图”、`UT-PORTRAIT`“报名照 / 底色头像”、`UT-TEMPLATE`“社交头像与封面”优先于自由工具出现。前两项逐次确认后复用现有抠图 Provider，并在本机修边、换底和导出；未配置服务时保持不可选。商品场景默认 1:1 / 1600 px 上限和白底 JPEG，不生成阴影、不保证平台审核；报名照场景不承诺护照、签证或任何机构规格。社交场景完全本地运行。注册表、初始设置、可用性、恢复和下载合同已有自动化覆盖，但新的三场景视觉排序和远程交互仍需浏览器复核。
