...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 08 typed case-context 与 actual-case driver 已完成 fake-only 验证 | `Slice 08: scope-frozen / typed-protocol-fake-tested / definition-not-created / Gate-B-smoke-not-run / calibration-forbidden / non-C1 / non-product` | 定向 12 / 12；Slice 07 保持 closed non-pass；新版本仍须完整执行 12 sources / 36 attempts，不拼接或补跑旧结果 |
...
| Slice 08 协议 | typed runner → actual-case driver 已实现并 fake-tested | 2 modules / 2 tests；targeted 12 / 12；计划 12 sources / 36 attempts；results=not-created | closed self-hashed context 绑定 disposition / code / source / manifest / gold；rejection worker-free；真实 Sharp 路径禁止到 results-zero definition 提交并推送之后 |
...
10. **Slice 08 typed protocol / actual-case driver 已通过 fake-only 12 / 12**：按 [Slice 08 范围合同](research/SLICE_08_CONTRACT.md) 新建 closed self-hashed `caseContext`，实际 callback 只接收 `{caseContext}`；source bytes / hash 与 gold 都需交叉绑定，rejection 不得读取 gold 或启动 worker。不是只补跑 18 个 rejection，而是在新版本、新 ID、新 request 下完整计划 normalize / export 各 18 次。当前 machine definition、result、decision 均未创建，calibration 继续禁止。
