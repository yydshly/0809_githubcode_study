...
6. 继续界面收敛：当前功能集合冻结为内部试用可用基线；先优化跨页面信息层级、结果工作区、导航和响应式可读性，不再围绕单个控件叠加微功能；
7. 在浏览器控制入口恢复后运行两档桌面视口、键盘主路径、synthetic 诊断页及真实 Chrome / Edge 下载 E2E，并据此完成基础编辑器早期内部可用性走查。
