...
Before any model artifact acquisition, the project must separately freeze dependency/runtime inventories, exact checkpoint URL and expected licensing material, resource bounds, and a no-network worker protocol. Natural-person evaluation requires a separately approved, non-user-upload research dataset with its own rights, privacy, isolation, and denominator records. No existing synthetic result can substitute for that population.
