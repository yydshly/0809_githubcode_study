...
1. `CAP-01 + CAP-02`：Slice 05 Gate B 双拒绝，Slice 06 diagnostic 已定位 Sharp PNG profile 缺陷；Slice 07 applicable 路径全过，但 rejection driver protocol 失配导致 Gate B 再次双拒绝。Slice 08 typed context + actual-case driver 已 fake-only 12 / 12，machine definition / result 尚未创建；calibration 与 C1 仍为 0。
