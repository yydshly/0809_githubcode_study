...
`UT-TEMPLATE` 现已把常用比例和导出上限组织为独立本地任务：1:1、4:5、16:9、9:16 均进入同一 normalized crop 与 renderer 合同，五个预设只组合比例和最长边上限；小图不放大，实际尺寸单独显示。当前不做主体感知裁切、扩图、平台规则同步、文字排版或发布检查，因此不得把按钮名称解释为平台合规证明。下一步在真实浏览器走查模板切换、长图 / 宽图拖动、手动覆盖、撤销和最终下载尺寸。
...
7. 继续界面收敛：基础链路冻结为内部试用可用基线；先优化跨页面信息层级、结果工作区、导航和响应式可读性，不再围绕单个控件叠加微功能；
