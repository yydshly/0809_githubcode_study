...
表中的“首批研究重点”是待验证范围，不是当前产品格式或能力清单。`CAP-02` 的 Slice 05–08 失败历史保持不可改写；Slice 09 的 canonical PNG normalize / export 唯一 open smoke 已双 Gate-B pass。Slice 10 在 1 / 288、Slice 11 在 0 / 288 attempts 处分别因本地协议错误 fail closed；Slice 11 未调用 Sharp worker或像素路径，同版本不得重跑。到此停止增加 normalize/export runner slice，转回 `CAP-03` SourceCard.v0 与 `CAP-04` Matting baseline。现有结果都不是 formal holdout、C1 或产品支持；JPEG / WebP 等行继续 fail closed，全部格式仍为 `productSupport=false`。
