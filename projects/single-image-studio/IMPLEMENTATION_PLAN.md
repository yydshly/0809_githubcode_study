...
立即后续禁止重跑 `@0.5.0` / `@0.6.0` / `@0.7.0` 或运行旧 calibration。[Slice 08 合同](research/SLICE_08_CONTRACT.md) 已把下一步收紧为新的 `@0.8.0` typed case-context evidence unit：先用 production callback shape 的 fake adversarial tests 证明 disposition / expected code / source / manifest fields 不会丢失，再冻结 results-zero definition。新 smoke 必须完整执行 36 次，不能只补跑 Slice 07 的 rejection。两项 Gate B 都通过前 calibration 继续禁止。
