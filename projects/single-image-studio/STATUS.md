...
| Slice 10 范围、协议、preview、runner、case adapter 与 registered-driver foundation | scope 合同已冻结；协议 7/7、preview 9/9、runner 7/7、case adapter 9/9、driver 5/5 | normalize / export 各 48 sources × 3 = 144 attempts；合计 96 / 288 | runner 只在全部 terminal 后接受 distinct durable runtime-end ref；driver 尚缺 runtime-end observer 与 central validator，正式 definition / result 与 formal holdout 均未创建。详见 [Slice 10 合同](research/SLICE_10_CONTRACT.md) |
...
12. **Slice 10 scope、协议、机器定义预览、durable runner、actual-case adapter 与 registered-driver foundation 已 fake-tested**：按 [Slice 10 合同](research/SLICE_10_CONTRACT.md) 计划 normalize / export 各 48 source × 3，共 96 source / 288 attempts；固定零 retry / replacement。协议 7/7、preview 9/9、runner 7/7、case adapter 9/9。case adapter 重开全部 96 个 Slice 05 source lineage 与 48 个 independent gold；48 个 rejection 均在 worker 前精确分类。registered-driver foundation 5/5，证明 central / Git / results-zero admission、单 invocation 顺序两项 operation、普通 complete non-pass 继续另一项，以及 protocol uncertainty 全局停止。runner 的 end-runtime 接口已改为全部 terminal 后才接收 distinct durable record ref，禁止预先把 start ref 冒充结束观测。durable runtime-end observer 与 central validator 尚未创建，driver 因而不可执行；preview 须吸收最终实现 hashes 后重新生成。正式 definition、results 与 formal holdout 仍未创建。
