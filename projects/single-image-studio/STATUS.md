...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 08 已不可重放地 protocol-failed；Slice 09 gold identity + actual driver 已 fake-tested | `Slice 09: scope-frozen / identity-context-driver-fake-tested / durable-runner-not-started / definition-not-created / Gate-B-smoke-not-run / calibration-forbidden / non-C1 / non-product` | 三套定向测试 14 / 14；normalize / export 均走真实 gold shape，但尚未建 durable runner / definition，不运行 Sharp、不创建结果 |
...
| Slice 09 实现 | gold identity、closed case context 与 actual-case driver 已实现；runner / definition / result 未创建 | targeted `14 / 14`；full verify `407 / 407`；计划 12 sources / 36 attempts；actual results `0` | applicable 仅从 typed identity 取 `goldRecordId` 与 expected facts；rejection gold-free / worker-free；两 operation 的 3+3×3 分母和旧 S08 replay 边界已 fake-tested；详见 [Slice 09 contract](research/SLICE_09_CONTRACT.md) |
