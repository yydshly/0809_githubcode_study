...
| `REG-NORM-SHARP-CANONICAL-PNG@0.9.0` | 相同像素 / encoder 架构 + versioned gold identity | `CAP-02` | canonical PNG normalize / export | `scope-frozen/definition-tooling-fake-tested` | identity / context / actual driver / durable runner / registered admission / generator / validator 通过 33 / 33；18 schemas / 12 sources / 6 identities / 36 planned attempts / copied bytes 0 / results 0 的预览树可逐 byte 重生；canonical definition / result 尚未创建，非 Gate B |
...
| `CC-CAP02-NORMALIZE-PNG@0.9.0` | `CAP-02` | canonical PNG source → future normalized artifact binding | planned `REG-NORM-SHARP-CANONICAL-PNG@0.9.0` | scope-only；新 gold identity 和完整 18-attempt operation 分母已冻结，implementation / definition / result 未创建，calibration=false、C1=0 |
| `CC-CAP02-EXPORT-PNG@0.9.0` | `CAP-02` | independent normalized input → future delivery artifact binding | planned `REG-NORM-SHARP-CANONICAL-PNG@0.9.0` | scope-only；不得重用 candidate normalize output，完整 18-attempt operation 分母待新定义，calibration=false、C1=0 |
