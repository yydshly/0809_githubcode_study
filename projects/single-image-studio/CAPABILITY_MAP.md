...
表中的“首批研究重点”是待验证范围，不是当前产品格式或能力清单。`CAP-02` 的 Slice 05–08 失败历史保持不可改写；Slice 09 的 canonical PNG normalize / export 唯一 open smoke 已双 Gate-B pass。Slice 10 冻结开放 calibration 范围，计划 96 个项目原创 synthetic source / 288 attempts；协议、definition preview 与 durable runner foundation 已 fake-tested，但 registered actual driver、正式 definition 和 results 尚未创建。即使后续 calibration 通过，也不是 formal holdout、C1 或产品支持。JPEG / WebP 等行继续 fail closed，全部格式仍为 `productSupport=false`。
