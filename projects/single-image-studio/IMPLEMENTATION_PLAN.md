...
Slice 06 Phase A 只冻结 [scope contract](research/SLICE_06_CONTRACT.md) 与 docs-only workspace。计划身份是 `REG-NORM-SHARP@0.6.0`、normalize / export 两份 `CapabilityContract@0.6.0` 与两份 operation-specific diagnostic preregistration；它们目前都没有 machine record。计划分母固定为 8 个既有 public-synthetic regression source unit，每项 3 次，共 24 次：normalize / export 各 3 个 opaque / partial-alpha / alpha-holes applicable lineage，另分别增加 missing-sRGB 与 invalid-artifact preflight sentinel。

Phase B 只能实现严格 diagnostic persistence、fake protocol tests 与新的 S06 error boundary；Phase C 才可 fresh inventory、以真实 UTC 冻结 results-0 definition，并在提交 / 推送后每个 operation 运行一次 registered characterization。所有 output 只可成为排除于 Gate B 的 diagnostic specimen / quarantine，并必须绑定完整 worker 与 independent-oracle 子记录。无论结果如何，Slice 06 都保持 `Gate-B-not-entered-diagnostic-only`、`calibrationAuthorized=false`；下一切片才可根据诊断选择 / 修正 candidate 并重新冻结 Gate-B smoke。Slice 06 不创建 calibration、正式 holdout、defect-holdout、escape、产品 UI / server 接线、真实图片或模型权重。
