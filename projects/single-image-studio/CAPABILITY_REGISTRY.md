...
| `REG-NORM-SHARP-CANONICAL-PNG@0.9.0` | 相同像素 / encoder 架构 + versioned gold identity | `CAP-02` | canonical PNG normalize / export | `scope-frozen/context-driver-fake-tested` | identity / context / actual driver 以真实 normalize / export `goldRecordId` shape 通过 14 / 14；3+3×3 分母、rejection worker-free、bytes / ref / manifest / replay 均 fail closed；durable runner / definition / result 为 0，非 Gate B |
