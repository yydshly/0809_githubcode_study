...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 06 诊断协议与 results-zero machine definition 已冻结；真实诊断尚未运行 | `scope-frozen / implementation-ready / definition-frozen / results-zero / diagnostic-characterization-not-run / Gate-B-no-authority / calibration-forbidden` | Slice 05 `@0.5.0` 继续保持 `smoke-closed-non-pass / Gate-B-denied-not-entered`，不得重跑或 calibration；[Slice 06 合同](research/SLICE_06_CONTRACT.md) 只授权在该精确 `@0.6.0` 定义所在提交保持 clean 且已推送并通过 admission 后运行一次 driver invocation（normalize / export 各一个 registered operation run，共 24 次计划 attempt），再由下一切片选择 candidate 并另行冻结 Gate-B smoke |
...
| Slice 06 诊断定义 | Phase C canonical definition 已冻结；`results/` 不存在 | 7 scripts；26 strict schemas；23 个非 index records；49 descendants / 50 machine files / 51-file full tree；2 manifests；8 lineage wrappers / 8 source units / 24 planned attempts；6 tests；Slice 06 `95 / 95`；全量 verify `323 / 323` | definition 于 `2026-08-15T08:17:06.288Z` 冻结；index content / file SHA-256 为 `d537199c8bc6147761da297daeddb03e1ff837a83c8d2c57af29c9e5b9b67e08` / `1cb934a1d870a62e9ccb706e3c21dcdbb54de55f027a325e31230ac4bf3cb20c`，full-tree SHA-256 为 `19a42a2e63b73fb0971e7038e4470c02551dce9b0ddba2dfc87dd3e5780d47b3`。没有复制图片 bytes、没有运行真实 Sharp、没有 result / artifact / calibration / Gate B decision / 正式 holdout / 产品接线；全部证据轴仍为 0 |
...
8. **已完成 Slice 06 Phase B 协议与 Phase C results-zero 定义冻结**：在 [Slice 06 合同](research/SLICE_06_CONTRACT.md) 的窄边界内建立 isolated worker、strict adapter、独立 PNG diagnostic oracle、durable runner、generator、central validator 与 admission / execution driver。canonical tree 含 26 份 strict schema、23 个非 index record、8 个只引用 Slice 05 公开 synthetic bytes 的 lineage wrapper，以及 fresh runtime / hardware；冻结时 `results/` 不存在、复制图片 bytes 为 0。中央对抗测试 `31 / 31`、Slice 06 六套测试 `95 / 95`、整仓 verify `323 / 323`，未运行真实 Sharp 图片路径。
9. **下一步只允许在已提交、已推送且 clean 的冻结定义上运行一次注册诊断**：执行 admission 必须逐项匹配 definition / runtime / implementation / Git 状态；一次 driver invocation 内按顺序最多注册 normalize / export 各一个 operation run，计划分母为 8 个 source × 3 = 24 attempts，零 replacement。首 operation 若 `protocol-failed` 或 `inconclusive` 会全局停止，不能补跑第二项；任何 drift、partial root 或 reconciliation unknown 都关闭该版本。输出只可保存为 diagnostic specimen / quarantine；Slice 06 没有 Gate B decision authority，始终 `calibrationAuthorized=false`，诊断闭合后仍由下一切片选择 / 修正 candidate 并另行冻结 Gate-B smoke。
