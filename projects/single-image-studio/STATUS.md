...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 10 已 fail closed；Slice 11 Phase B protocol + durable foundation 已实现 | `Slice 11: scope-frozen / projection-case-lifecycle-protocol-and-durable-foundation / definition-not-created / calibration-not-run / non-C1 / non-product` | projection、case、lifecycle、request / terminal / ledger / summary、durable claim / runtime observation 与六文件原子发布通过 27 / 27 fake-only tests；真实 calibration 仍须等待 registered durable driver、central validator 与新 results-zero definition 提交推送 |
...
| Slice 11 foundation | `@0.11.0`；projection / adapter / case / lifecycle / protocol runner / durable publication primitives 已实现，definition / results 未创建 | 新身份的 96 sources / 288 attempts；零 retry / replacement | 10 strict schemas；terminal 引用独立 lifecycle / projection；claim 幂等；六文件 closure 在 intent 后原子 rename；post-rename 异常为 reconciliation-unknown；27 / 27 fake-only。详见 [Slice 11 合同](research/SLICE_11_CONTRACT.md) |
...
13. **Slice 11 protocol + durable foundation 已实现，definition 尚未创建**：`@0.11.0` 的 projector、adapter 与 case executor 保留完整 gold identity并只向底层下发 exact 12 fields；strict lifecycle record 独立记录 preflight、spawn、IPC 与 exit。纯内存 runner 生成 versioned request / terminal / ledger / summary，固定每 operation 48×3、零 replacement。新增 durable claim、start/end runtime observation、oracle facts 与六文件 applicable closure：文件 fsync 后记录 intent，再原子 rename；pre-rename 失败清理 staging，post-rename 失败保留唯一 closure 并 fail closed 为 reconciliation-unknown。27 / 27 fake-only tests覆盖重哈希、串线、分母、majority、claim 冲突和发布崩溃窗口。下一步仍需 registered durable driver、central validator 与 machine definition；冻结并推送 results-zero baseline 后才可能运行。它仍不是 C1 或产品支持。
