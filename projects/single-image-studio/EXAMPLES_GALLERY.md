...
## 2026-08-18 · 老照片差异可见性修复

用户反馈老照片前后效果在样例卡中不明显。浏览器复现确认两个原因：本地卡使用的“褪色提层次”平均通道差仅约 `6.36`；静态生成式参考仍可能在 async/lazy 解码末尾短暂显示空棋盘格。

修复后：

- 本地卡改用产品现有“黑白层次”预设，不新增或夸大算法能力；
- 标题和摘要明确它只改变已有像素，不会去划痕、修脸或补画；
- 两张旧照片卡优先加载，并在图片可用前显示明确加载状态；
- runtime 结果必须 `decode()` 完成后才进入 ready；
- 本地结果与静态参考都提供“单独查看结果”；
- 生成式参考文案引导比较顶部横向划痕、斑点、褪色以及脸 / 自行车 / 砖墙变化。

隔离 Chrome run `9f5e9f31-4b7c-4d8e-ae21-223344556677` 在 1180 px / 390 px 以 `2/2` 通过：18 张图片全部完成解码，本地老照片结果平均通道差 `10.67`、变化覆盖 `98.4%`，无横向溢出。该差异只证明样例能展示真实本地黑白层次变化，不代表生成式修复已成为产品能力。

随后用户发现多张普通样例长期停在“正在加载已登记图片…”。根因是 lazy 图片同时被设为 `hidden`，隐藏元素无法进入视口，形成加载停滞。现在只有尚无 URL 的 runtime 结果保持 hidden；已有路径的 lazy 图片始终参与布局和加载，状态文案仅在 load 事件后关闭。`a06fa042-5c8d-4e9f-bf32-334455667788` 再次以 2 / 2 核对 18 张图片完整解码。

“分享前清理 metadata”也已改名为“清理图片文件信息（画面基本不变）”，明确它是文件安全 / 交付能力而非视觉特效：移除受控 EXIF / GPS 等 metadata、限制尺寸和体积，画面预期基本不变，也不会识别画面中的人脸、地址、车牌或文字。
