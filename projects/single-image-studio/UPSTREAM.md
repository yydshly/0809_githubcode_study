...
Slice 10 新增的 scope、协议、definition、runner、case adapter、driver、observer 与 validator 均为本地原创研究代码，只复用已锁定的 Sharp runtime 和既有项目原创 Slice 05 open-synthetic records；没有新增 dependency、第三方代码或资产、真实图片、模型、权重、formal holdout 或产品接线。results-zero definition 已先提交推送，随后唯一 registered invocation 只写入 1 request、1 claim、1 terminal 与 2 个 ledger events：完整 gold expected 与 Slice 07 adapter 的 12-field expected schema不兼容，故在 Sharp worker 前以 `S10_EXPECTED_OUTPUT_INVALID` 停止，没有生成 candidate output、artifact 或 oracle record。该运行还暴露了本地错误映射把 pre-worker 路径误记为 `workerInvoked=true`；中央验证因此 fail closed。结果原样保留且不得重跑，任何修复必须进入 Slice 11 新版本；这不改变 upstream、许可或产品支持状态。
...
| `REG-MATTE-MODNET@0.1.0` | [ZHKKKe/MODNet commit `28165a451e4610c9d77cfdf925a94610bb2810fb`](https://github.com/ZHKKKe/MODNet/tree/28165a451e4610c9d77cfdf925a94610bb2810fb) | Apache-2.0 | 官方 `pretrained/README.md` 指向 Google Drive folder，但 direct object / bytes / SHA-256 未解析；image demo imports 与 pickle-compatible `torch.load` 已登记，runtime / loader / checkpoint 均未创建或请求 | 无 |
| `REG-MATTE-RVM-MOBILENETV3@0.1.0` | [PeterL1n/RobustVideoMatting v1.0.0 / `17d1774b032fd503bfe53c57d295db719f9e3da1`](https://github.com/PeterL1n/RobustVideoMatting/tree/17d1774b032fd503bfe53c57d295db719f9e3da1) | GPL-3.0 | 官方 release URL、hubconf 与 exact direct requirements 已登记；model bytes / SHA、Python / platform wheels / transitive hashes / safe loader 均未取得，任何产品分发先做 GPL / derivative / source-offer review | 无 |
...
`observed_at=2026-08-16` 的获取治理复核先确认固定 MODNet root README 只公开 Colab demo；后续 runtime metadata step 又从同一固定 commit 的 `pretrained/README.md` 解析到官方 Google Drive folder。该 folder 仍不提供可冻结的 direct object URL、byte length 或 SHA-256，因此 `modnet_photographic_portrait_matting.ckpt` 保持硬阻断。RVM v1.0.0 release 入口与 `rvm_mobilenetv3.pth` direct path、fixed requirements 已登记，但没有对 asset 发 HEAD/GET，byte length / SHA-256 仍 pending。两项计划及 offline runtime policy 见 [Matting acquisition governance evidence](./research/MATTING_ACQUISITION_GOVERNANCE_EVIDENCE.md)，后续固定源码元数据、安全 loader 与未签发请求模板见 [Matting runtime metadata evidence](./research/MATTING_RUNTIME_METADATA_EVIDENCE.md)。
