...
立即后续禁止重跑 `@0.5.0`–`@0.10.0` 或补跑旧 calibration。Slice 11 / `@0.11.0` scope 已冻结，projection / case / lifecycle / protocol runner / durable operation foundation 已实现：完整 gold identity 被投影为 exact 12 adapter keys；strict lifecycle record、versioned request / terminal、成对 hash-chain ledger 与 48×3 summary 全部交叉绑定。durable bridge 先持久 request / claim，再为 applicable、worker-free rejection 或 failure 写入对应 closure；每次原子 rename 和 completion callback 结束前不允许下一 worker。operation 层以 288-event fsynced publication hash chain、runtime start/end observation 与 atomic final close 收口。37 / 37 fake-only tests验证完整 48×3 分母、重放阻断、多种 closure、global stop、publication reconciliation 与 operation close，且不调用 Sharp。下一步只实现 central adversarial validator，再以新 identities 重建 96 / 288 的 results-zero definition并推送；在此之前不得运行真实 calibration。
