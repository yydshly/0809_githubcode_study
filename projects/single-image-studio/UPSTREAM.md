...
Slice 11 当前新增本地原创 scope 合同、expected projector、versioned lifecycle adapter、strict lifecycle record、generic case executor、durable attempt / operation runner、registered driver、definition generator、central validator、fake-only tests 与 results-zero machine metadata；没有新增 dependency、upstream package、第三方代码 / 图片 / 模型 / 权重或产品接线。42 / 42 tests 只用内存 gold data、fake child process 与系统临时 closure tree，不运行 Sharp。canonical definition 只建立 96 个新 source identity、48 gold identities、4 manifests 与23 schemas，引用既有项目原创 public-synthetic material，复制图片 bytes=0、request/result=0。真实 calibration 仅可在该 definition commit 推送且工作树clean后运行一次。
