...
立即后续禁止重跑 `@0.5.0`–`@0.10.0` 或补跑旧 calibration。Slice 11 / `@0.11.0` scope 已冻结，projection / case / lifecycle / protocol runner / durable publication foundation 已实现：完整 gold identity 被投影为 exact 12 adapter keys；strict lifecycle record、versioned request / terminal、成对 hash-chain ledger 与 48×3 summary 全部交叉绑定。durable primitives 加入幂等 `wx` claim、start/end runtime observation、oracle facts 及六文件原子发布，并在 rename 前后失败窗口 fail closed。27 / 27 fake-only tests验证分母、3/3 byte determinism、worker-free rejection、重哈希攻击、global stop、claim 冲突和 publication reconciliation，且不调用 Sharp。下一步须实现 registered durable driver 与 central adversarial validator，再以新 identities 重建 96 / 288 的 results-zero definition并推送；在此之前不得运行真实 calibration。
