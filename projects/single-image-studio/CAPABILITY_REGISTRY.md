...
| `REG-NORM-SHARP-CANONICAL-PNG@0.11.0` | Slice 10 immutable failure lineage + versioned expected projection / truthful lifecycle | `CAP-02` | canonical PNG normalize / export | `scope-frozen/fake-only-foundation` | exact 12-field projection、versioned adapter、strict lifecycle record 与 generic case executor 已通过 13 / 13 fake-only tests；96/288 definition、request、runner 与 results 尚未创建；零 retry/replacement，非 C1、非产品支持 |
...
8. Slice 11 / `@0.11.0` scope 已冻结；content-addressed gold-to-adapter exact 12-field projector、事实派生的 lifecycle adapter、strict self-hashed lifecycle record 与 frozen-material case executor 已通过 13 / 13 fake-only tests。它不调用 Sharp，并能区分 preflight-not-started、spawn-attempted、IPC 与 confirmed exit；rejection 保持 worker-free。request / runner / machine definition 与 results 仍未创建；完整实现和 results-zero 基线分别提交推送前不得运行。
