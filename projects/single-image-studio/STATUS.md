...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 06 唯一注册诊断已闭合；Slice 07 fake-tested runtime protocol 已完成 | `Slice 07: scope-frozen / implementation-ready / definition-not-frozen / Gate-B-smoke-not-run / calibration-forbidden / non-C1 / non-product` | encoder、Sharp raw worker、adapter、durable runner 与 6 份动态 strict schema 已完成 fake-only tests；machine definition 与 result 均未创建，真实 Slice 07 Sharp 图片路径未运行 |
...
| Slice 07 新候选实现 | scope 已冻结；candidate runtime protocol 已完成 fake-only `25 / 25` | encoder `cf71ddd2…c5d78`；raw worker `4ce6b725…f30bd`；adapter `88c05432…af61e`；runner `6decec76…a71f8` | encoder 经 independent oracle 重开；worker 只返回 RGBA8，adapter 要求合法 IPC + clean exit，runner 固定 6 sources × 3、原子闭包与 hash-chain ledger。definition / results 仍为 0。详见 [Slice 07 workspace](research/slice-07/README.md) |
...
9. **Slice 07 scope 与 fake-tested candidate protocol 已完成，不得重跑 Slice 06**：新版本选择 Sharp 解码 / 像素处理 + candidate-owned canonical PNG encoder，且生产 encoder 与 oracle 实现隔离。raw worker、adapter、durable runner 和 strict runtime schemas 已落盘，但只在 fake / 内存 synthetic tests 中执行；下一步冻结 results-zero definition。只有定义基线提交并推送后才允许一次 12-source / 36-attempt registered Gate-B smoke。当前 Gate B、calibration、C1、产品支持继续为 0 / false。
