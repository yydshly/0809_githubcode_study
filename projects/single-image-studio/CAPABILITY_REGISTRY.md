...
| `REG-NORM-SHARP-CANONICAL-PNG@0.11.0` | Slice 10 immutable failure lineage + versioned expected projection / truthful lifecycle | `CAP-02` | canonical PNG normalize / export | `scope-frozen/fake-only-protocol-and-durable-operation-foundation` | protocol runner、durable request / claim / applicable-rejection-failure publication bridge、288-event fsynced ledger 与 atomic operation close 通过 37 / 37 fake-only tests；96/288 definition 与 canonical results 尚未创建；零 retry/replacement，非 C1、非产品支持 |
...
8. Slice 11 / `@0.11.0` scope 已冻结；projection、事实派生 lifecycle、frozen-material case executor、versioned request / terminal / ledger / summary runner，以及 durable request / claim / runtime observation / oracle facts / atomic attempt bridge / operation close 已通过 37 / 37 fake-only tests。它不调用 Sharp；applicable、worker-free rejection 与 failure 分别只能发布各自的 exact closure，发布完成前不允许下一 worker，post-rename 不确定性全局停止，重放不获得执行权。每个 operation 另以 288-event fsynced publication hash chain、runtime start/end 与 atomic final close 收口。machine definition 与 canonical results 仍未创建；central validator 和 results-zero 基线分别提交推送前不得运行。
