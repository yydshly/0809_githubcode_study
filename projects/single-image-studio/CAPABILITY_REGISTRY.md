...
| `REG-NORM-SHARP-CANONICAL-PNG@0.9.0` | 相同像素 / encoder 架构 + versioned gold identity | `CAP-02` | canonical PNG normalize / export | `scope-frozen/identity-layer-fake-tested` | closed identity 以真实 Slice 05 `goldRecordId` shape 通过 6 / 6；fallback / dual-key / bytes / ref / operation / rejection-gold 均 fail closed；完整 driver / definition / result 为 0，非 Gate B |
