...
Slice 11 当前新增本地原创 scope 合同、expected projector、versioned lifecycle adapter、strict lifecycle record、generic case executor、fake-only tests 与 workspace README；没有新增 dependency、upstream package、第三方代码 / 图片 / 模型 / 权重、machine definition、result 或产品接线。13 / 13 tests 只用内存 gold data、fake child process 与内存 case material，不运行 Sharp。它只能引用既有项目原创 public-synthetic material 与已锁定 runtime lineage，后续任何真实 calibration 都必须等待完整 runner 协议与新的 results-zero definition 分别提交推送。
