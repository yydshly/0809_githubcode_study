...
立即后续禁止重跑 `@0.5.0`–`@0.9.0` 或运行旧 calibration。Slice 09 唯一 registered smoke 形成 36 / 36 terminal pass、18 artifact closures 与两项 Gate-B pass，结果 commit 为 `c91014c6bef8878277a8520d003b10684972087b`；这只完成开放 research Gate B，C1=0、productSupport=false。[Slice 10 calibration 范围](research/SLICE_10_CONTRACT.md) 冻结为 normalize / export 各 48 sources × 3、合计 96 / 288，零 retry / replacement；协议 7/7、22-schema preview 9/9、runner 7/7、case adapter 9/9、registered-driver foundation 5/5、runtime-end observer 6/6。preview 已吸收并 pin 完整 runner / case / driver / observer 实现。runner 只在全部 terminal 后接收 distinct durable runtime-end ref；driver 证明 central / Git / results-zero admission、两项顺序 operation、普通 non-pass 继续和 protocol 全局停；observer 重新盘点冻结 runtime 并以 `wx` + fsync 发布独立 end record，漂移、嵌入 payload 伪造与重放均关闭失败且不处理图片。central validator、正式 definition 与 results 仍未创建，driver 不可执行；提交推送前不得运行 calibration。
