...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 10 已 fail closed；Slice 11 Phase B foundation 已实现 | `Slice 11: scope-frozen / projection-case-and-lifecycle-foundation / definition-not-created / calibration-not-run / non-C1 / non-product` | exact 12-field projection、versioned adapter、strict lifecycle record 与 generic case executor 通过 13 / 13 fake-only tests；真实 calibration 仍须等待完整 runner 协议与新 results-zero definition 提交推送 |
...
| Slice 11 foundation | `@0.11.0`；projection / adapter / lifecycle record / case executor 已实现，definition / results 未创建 | 新身份的 96 sources / 288 attempts；零 retry / replacement | 投影保留完整 gold hash并只下发 12 keys；case 层绑定 frozen material；preflight / spawn / IPC / exit 生命周期成为 strict self-hashed record；13 / 13 fake-only。详见 [Slice 11 合同](research/SLICE_11_CONTRACT.md) |
...
13. **Slice 11 projection / case / lifecycle foundation 已实现，definition 尚未创建**：`@0.11.0` 的 content-addressed projector 保留完整 gold canonical JSON/hash，只把 exact 12 fields交给 adapter；generic case executor 把完整 gold 仅交给 versioned adapter，并将 request 精确绑定 frozen case material；strict self-hashed lifecycle record 独立记录 preflight、spawn、IPC 与 exit。13 / 13 fake-only tests 覆盖 projection、schema、自重哈希洗白、worker-free rejection、request rebinding 与成功闭包。下一步仍需完整 request / terminal / ledger / summary / runner / central definition，冻结并推送 results-zero baseline 后才可能运行；它仍不是 C1 或产品支持。
