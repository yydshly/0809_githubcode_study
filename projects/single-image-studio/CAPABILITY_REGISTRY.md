...
| `REG-NORM-SHARP-CANONICAL-PNG` | Sharp RGBA8 pixel stage + 项目原创 canonical PNG encoder | `CAP-02` | canonical PNG normalize / export | `definition-frozen/results-zero`（非 Gate B） | 16 schemas、23 non-index records、12 wrappers、36 planned attempts；definition full-tree `bc608ed9…0481`，Gate-B result 未创建、真实 Sharp 图片路径未运行，两项 Gate B 全过前 calibration 禁止 |
...
| `CC-CAP02-NORMALIZE-PNG@0.7.0` | `CAP-02` | canonical PNG source → future `NormalizedImage` v0.7 binding | frozen `REG-NORM-SHARP-CANONICAL-PNG@0.7.0` definition | machine contract / prereg / 3 applicable + 3 rejection × 3 已冻结；Gate-B result 未创建，C1=0 |
| `CC-CAP02-EXPORT-PNG@0.7.0` | `CAP-02` | independent normalized input → future `DeliveryArtifact` v0.7 binding | frozen `REG-NORM-SHARP-CANONICAL-PNG@0.7.0` definition | 与 normalize 共用候选架构但独立 decision；machine contract / prereg / 3 applicable + 3 rejection × 3 已冻结，两项都 pass 前 calibration 禁止，C1=0 |
...
4. Slice 07 已冻结 `REG-NORM-SHARP-CANONICAL-PNG@0.7.0` results-zero definition：Sharp 只产出 RGBA8，项目原创 encoder 写 closed PNG profile，independent oracle 只验收。result 仍为 0；先提交并推送该定义，才允许一次 36-attempt registered smoke。
