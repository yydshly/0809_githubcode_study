...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 10 已 fail closed；Slice 11 results-zero definition 已冻结 | `Slice 11: definition-frozen / results-zero / calibration-not-run / non-C1 / non-product` | 23 schemas、96 source wrappers / 288 planned attempts、0 copied image bytes；central validator 与 registered driver 已实现，42 / 42 Slice 11 fake-only tests通过；真实 calibration 仍须等待本 definition commit 推送且工作树 clean |
...
| Slice 11 definition | `@0.11.0`；freeze `2026-08-15T23:01:50.529Z`；index `10676a…33bba` / file `04d2f8…8266` | 新身份的 96 sources / 288 attempts；零 retry / replacement；results=0 | 23 strict schemas；96 source wrappers、48 gold identities、4 manifests、0 copied image bytes；full tree `d33d25…2e90c`；42 / 42 fake-only。详见 [合同](research/SLICE_11_CONTRACT.md) 与 [evidence](research/SLICE_11_EVIDENCE.md) |
...
13. **Slice 11 results-zero definition 已冻结，尚未运行**：`@0.11.0` 的 projector、adapter、case executor、durable attempt / operation runner、registered driver 与 central validator 已闭合。definition 于 `2026-08-15T23:01:50.529Z` 冻结，含 23 schemas、96 个新 source wrapper、48 gold identities、4 manifests、两份 contract / plan 与 fresh runtime，复制图片 bytes=0、results=0。每 operation 固定 48×3、零 replacement；publication 必须形成 288-event fsynced hash chain与 atomic final close。42 / 42 Slice 11 fake-only tests通过。只有本 definition commit 推送且 `HEAD == origin/main`、worktree clean 后，才允许唯一 registered calibration；它仍不是 C1 或产品支持。
