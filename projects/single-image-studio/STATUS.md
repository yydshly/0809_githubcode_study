...
| 维度 | 当前状态 | 数量 / 等级 | 这代表什么 |
| --- | --- | --- | --- |
| 研究阶段 | Slice 06 诊断协议实现与 fake-only 测试已就绪，定义尚未冻结 | `scope-frozen / implementation-and-fake-protocol-tests-ready / definition-not-frozen / diagnostic-characterization-not-run / Gate-B-no-authority / calibration-forbidden` | Slice 05 `@0.5.0` 继续保持 `smoke-closed-non-pass / Gate-B-denied-not-entered`，不得重跑或 calibration；[Slice 06 合同](research/SLICE_06_CONTRACT.md) 只授权在新 `@0.6.0` results-zero 定义提交并推送后运行 24 次开放诊断表征，再由下一切片选择 candidate 并另行冻结 Gate-B smoke |
...
| Slice 06 诊断协议 | Phase B fake-only 实现已就绪；canonical definition 与 results 为 `not-created` | 4 scripts；3 个落盘 oracle record schemas + 10 个 runner-exported strict schema documents；3 tests；pre-definition lifecycle patch 后 fake-only `46 / 46`；此前完整基线 verify `270 / 270`，Phase C 冻结后再记录新全量计数 | adapter / worker / independent diagnostic oracle / durable local runner 已实现并通过协议负例；测试只在内存构造 synthetic PNG bytes、在系统临时目录构造 fake closure，未运行真实 Sharp 图片路径。candidate / contracts / plans / preregistrations / manifests / runtime / hardware / fixture wrappers / definition index / results 均仍为 `not-created`。本切片没有 Gate B decision authority，不创建 calibration、artifact、正式 holdout 或产品接线 |
...
- Slice 06 两阶段 diagnostic characterization 授权、24 次计划分母、Phase B 协议实现状态与禁止边界见 [research/SLICE_06_CONTRACT.md](research/SLICE_06_CONTRACT.md) 和 [research/slice-06/README.md](research/slice-06/README.md)。
...
8. **已完成 Slice 06 Phase B 诊断协议实现**：在 [Slice 06 合同](research/SLICE_06_CONTRACT.md) 的窄边界内新增 isolated worker、strict adapter、独立 PNG diagnostic oracle、durable runner、3 份落盘 record schema 与 runner 导出的 10 份 strict schema document；pre-definition lifecycle patch 后 fake-only oracle / runner / runtime 合计 `46 / 46`，独立审计 `P1 / P2 / P3 = 0 / 0 / 0`。这些测试没有运行真实 Sharp 图片路径；新 `REG-NORM-SHARP@0.6.0`、两份 `CapabilityContract@0.6.0`、两份 preregistration、8 个 source wrapper、fresh runtime / hardware 与 definition index 仍为 `not-created`。
9. **下一步先冻结 Slice 06 results-zero 定义，不直接运行诊断或重启 Gate B**：Phase C 建立 generator / central validator、fresh runtime inventory、machine records 与 8 个既有公开 synthetic lineage wrapper，完成对抗审计后以真实 UTC 冻结、提交并推送 results-zero definition；之后才可为 normalize / export 各运行一次注册 characterization。该切片只保存 output specimen、exact oracle 子因、worker observation 与 exit state；没有 Gate B decision authority，始终 `calibrationAuthorized=false`。诊断闭合后由下一切片选择 / 修正 candidate，并另行冻结 Gate-B smoke；不得修补或重跑 `@0.5.0`。
